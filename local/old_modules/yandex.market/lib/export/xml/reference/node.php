...
abstract class Node
{
	...
	protected function refreshParameters()
	{
		...
		$this->overrides = isset($parameters['overrides']) ? (array)$parameters['overrides'] : null;
	}
	...
	public function validate($value, array $context, $siblingsValues = null, Market\Result\XmlNode $nodeResult = null, $settings = null)
	{
		...
		else if ($this->overrides !== null && in_array($value, $this->overrides, true))
		{
			$result = true;
		}
		else
	...
	{
		return $this->formatValueByType($value, $context);
	}
	...
	{
		if ($this->overrides !== null && in_array($value, $this->overrides, true) !== false)
		{
			$result = $value;
		}
		else
		{
			$result = $this->formatValueByType($value, $context, $nodeResult);

			if ($this->overrides !== null && isset($this->overrides[$result]))
			{
				$result = $this->overrides[$result];
			}
		}
	...
	protected function formatValueByType($value, array $context = [], Market\Result\XmlNode $nodeResult = null)
	{
		$valueType = $this->getValueType();
		$typeEntity = Market\Type\Manager::getType($valueType);
