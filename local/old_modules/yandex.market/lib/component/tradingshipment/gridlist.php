...
class GridList extends Market\Component\Base\GridList
{
	...
	protected function getStatusEnum()
	{
		...
		$types = [
			...
			'OUTBOUND_CREATED',
			'OUTBOUND_CONFIRMED',
			'MOVEMENT_COURIER_FOUND',
			'MOVEMENT_HANDED_OVER',
			'MOVEMENT_DELIVERING',
			'MOVEMENT_DELIVERED',
			'INBOUND_ARRIVED',
			'INBOUND_ACCEPTANCE',
	...
	public function load(array $queryParameters = [])
	{
		try
		{
			...
			if ($lastPage !== null)
			{
				$filterKey = $this->makeFilterKey($fetchParameters);

				$this->setPageToken($filterKey, $lastPage, $response->getField('nextPageToken'));
			}
			else
			{
	...
	protected function convertQueryToFetchParameters($queryParameters)
	{
		...
		{
			$result['page'] = floor($queryParameters['offset'] / static::PAGE_SIZE) + 1;

			if ($result['page'] > 1)
			{
				$result['pageToken'] = $this->getPageToken(
					$this->makeFilterKey($result),
					$result['page']
				);
			}
		}
	...
	protected function makeFilterKey(array $filter)
	{
		$parts = [];
	...
	protected function setPageToken($filterKey, $page, $token)
	{
		$sessionKey = $this->getPageTokenSessionKey();
		$queryKey = $filterKey . '-' . $page;
	...
	protected function getPageToken($filterKey, $page)
	{
		$sessionKey = $this->getPageTokenSessionKey();
		$queryKey = $filterKey . '-' . $page;
	...
	{
		return $this->getComponentParam('GRID_ID') . '_PAGE_TOKEN';
	}
