		...
		TradingService\Common\Concerns\Options\BuyerProfileInterface
{
	...
	protected function applyOrderUserRuleValues()
	{
		...
		{
			$this->values['ORDER_USER_RULE'] = TradingService\Common\Concerns\Options\UserRegistration::getDefault();
		}
	}
	...
	protected function getOrderUserRuleFields(TradingEntity\Reference\Environment $environment, $siteId)
	{
		...
		if (isset($result['ORDER_USER_RULE']['VALUES']) && !empty($disabled))
		{
			$disabledMap = array_flip($disabled);

			$result['ORDER_USER_RULE']['VALUES'] = array_filter($result['ORDER_USER_RULE']['VALUES'], static function($option) use ($disabledMap) {
