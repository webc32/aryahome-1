...
<?else:?>
	...
	<div class="detail <?=($templateName = $component->{'__template'}->{'__name'})?> fixed_wrapper">
		...
		<?if(in_array('LINK_GOODS', $arParams['DETAIL_PROPERTY_CODE'])):?>
		    ...
		    $dbProperty = CIBlockProperty::GetList(array(), array("IBLOCK_ID" => $catalogID, "CODE" => "LINK_NEWS"));
		    if($dbProperty->SelectedRowsCount() && $arElement['ID'])
		    {
			    $arTmpElement = CMaxCache::CIblockElement_GetList(array('CACHE' => array('TAG' => CMaxCache::GetIBlockCacheTag($catalogID), 'MULTI' => 'Y')), array('IBLOCK_ID' => $catalogID, 'PROPERTY_LINK_NEWS' => $arElement['ID']), false, false, array('ID'));
			    if($arTmpElement)
			    {
