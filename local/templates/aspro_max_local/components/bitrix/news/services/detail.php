...
<?else:?>
	...
	<div class="detail <?=($templateName = $component->{'__template'}->{'__name'})?> fixed_wrapper">
		...
		<?if(in_array('LINK_GOODS', $arParams['DETAIL_PROPERTY_CODE'])):?>
		    ...
		    {
			    $arTmpElement = CMaxCache::CIblockElement_GetList(array('CACHE' => array('TAG' => CMaxCache::GetIBlockCacheTag($catalogID), 'MULTI' => 'Y')), array('IBLOCK_ID' => $catalogID, 'PROPERTY_SERVICES' => $arElement['ID']), false, false, array('ID'));
			    if($arTmpElement)
			    {
		...
		<?//element?>
		<?if($bFastViewMode):?>
			<?$sViewElementTemplate = $arTheme["USE_FAST_VIEW_SERVICES"]["VALUE"];?>
		<?else:?>
