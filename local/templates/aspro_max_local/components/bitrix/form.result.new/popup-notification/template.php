...
<div class="form <?=$arResult["arForm"]["SID"]?>  ">
	...
	<?if(strlen($arResult["FORM_NOTE"])){?>
		<div class="form_result <?=($arResult["isFormErrors"] == "Y" ? 'error' : 'success')?>">
			...
			<?else:?>
                <?
                $requestParams = json_decode($_REQUEST['data-trigger'], true);
                $productId = $requestParams['data-autoload-product_id'];
                $productName = $requestParams['data-autoload-product_name'];


                // $productName =
              //  $requestParams->data-autoload-product_id;
                ?>
                <script>
                    add2waitingList('<?=$productId?>','<?=$productName?>');
                </script>
