...
<?if($iSectionsCount):?>
	<?$this->SetViewTarget("top_content");?>
		<?global $arTheme,${$arParams["FILTER_NAME"]}?>
		<?
		$preFilter = array();
				if($SMART_FILTER_FILTER["=PROPERTY_85"]){
					$preFilter = array(
						"PROPERTY_COLOR_FILTER" => $SMART_FILTER_FILTER["=PROPERTY_85"]
					);
				}
				if($SMART_FILTER_FILTER["=PROPERTY_75"]){
					$preFilter = array(
						"PROPERTY_ALLRAZMER_FILTER" => $SMART_FILTER_FILTER["=PROPERTY_75"]
					);
				}
				...
				if($_POST['offer_ajax'] && ($_POST['product_color'] || $_POST['product_size'])){
					...
					${$arParams["FILTER_NAME"]} = array("ID" => $id);
				}
		?>
