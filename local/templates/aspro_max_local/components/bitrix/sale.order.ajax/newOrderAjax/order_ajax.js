...
(function() {
    ...
    BX.Sale.OrderAjaxComponent = {
        ...
        sendRequest: function(action, actionData)
        {
            ...
            {
                var selectedGroup = this.getData(eventArgs.action, eventArgs.actionData)['order']['DELIVERY_GROUP'];
                BX.ajax({
                    method: 'POST',
                    dataType: 'json',
                    url: this.ajaxUrl,
                    ...
                    onsuccess: BX.delegate(function(result) {
                        ...
                        if(this.firstLoadToHide){
                            $('.deliveries').hide();
                            $('.bx-soa-customer-field[data-property-id-row="7"]').hide();
                            $('.bx-soa-customer-field[data-property-id-row="26"]').hide();
                            $('.bx-soa-customer-field[data-property-id-row="74"]').hide();
                            $('.bx-soa-customer-field[data-property-id-row="75"]').hide();
                            $('.bx-soa-customer-field[data-property-id-row="4"]').hide();
                        }
        ...
        showDeliveryItemsPage: function(page)
        {
            ...
            {
                deliveryItemNode = this.createDeliveryItem(this.deliveryPagination.currentPage[k],false);
        ...
        clickOrderSaveAction: function(event)
        {
            if($('#pvz_link').html() != 'Выбрать пункт выдачи'){
                $('[name=ORDER_PROP_30]').val($('#pvz_link').html());
            }
        ...
        getNextSection: function(actionSection, skippedSection)
        {
            ...
            var allSections = this.orderBlockNode.querySelectorAll('.bx-soa-section.bx-active:not(.region)'),
                nextSection, i;
        ...
        editSection: function(section)
        {
            ...
            var active = true, //var active = section.id == this.activeSectionId,
                titleNode = section.querySelector('.bx-soa-section-title-container'),
                editButton, errorContainer;

            if(section.id == "bx-soa-pickup"){
        ...
        editActiveRegionBlock: function(activeNodeMode)
        {
            ...
            else
            {
                ...
                //местоположение в доставке
                // var deliveryBlock = $('#bx-soa-delivery')[0];
                // var hasLocationInDeliverySect = $('#bx-soa-delivery .bx-soa-location-input-container').length != 0;
                // if (!hasLocationInDeliverySect){
                //  this.getDeliveryLocationInput(deliveryBlock);
                // }
        ...
        createPaySystemItem: function(item)
        {
            ...
            label = BX.create('DIV', {
                props: {className: 'bx-soa-pp-company-graf-container orderLabel'},
                children: [
                    BX.create('INPUT', {
                        props: {
            ...
            itemNode = BX.create('DIV', {
                props: {className: 'bx-soa-pp-company col-lg-6 col-sm-6 col-xs-6'},
                children: [label],
                events: {
        ...
        editActiveDeliveryBlock: function(activeNodeMode)
        {
            ...
            else
            {
                ...
                if((curDeliveryName.indexOf('курьер') + 1) || curDeliveryName.indexOf('Курьер') + 1 || curDeliveryName.indexOf('КУРЬЕР') + 1){
                    ...
                    for(var i = 0;i<this.deliveryPropsArray.length;i++){
        ...
        selectDeliveryGroup: function(event)
        {
            ...
            if(targetText == "Доставить курьером"){
                $('.bx-soa-customer-field[data-property-id-row="7"]').css("display",'block');
                $('.bx-soa-customer-field[data-property-id-row="26"]').css("display",'block');
                $('.bx-soa-customer-field[data-property-id-row="74"]').css("display",'block');
                $('.bx-soa-customer-field[data-property-id-row="75"]').css("display",'block');
                $('.bx-soa-customer-field[data-property-id-row="4"]').css("display",'block');
            }else{
                $('.bx-soa-customer-field[data-property-id-row="7"]').hide();
                $('.bx-soa-customer-field[data-property-id-row="26"]').hide();
                $('.bx-soa-customer-field[data-property-id-row="74"]').hide();
        ...
        editDeliveryInfo: function(deliveryNode,curDeliv)
        {
            if (!this.result.DELIVERY)
                return;
            ...
            while (group = groupIterator())
            {
                ...
                while (property = propsIterator())
                {
                    if (property.getName()=='Город') { //Если свойство совпадает с названием поля адреса в вашей системе
        ...
        editPropsItems: function(propsNode)
        {
            ...
            while (group = groupIterator())
            {
                ...
                while (property = propsIterator())
                {
                    ...
                    if(property.getId() != 4 && property.getId() != 7 && property.getId() != 74 && property.getId() != 75 && property.getId() != 26 && property.getId() != 22){
                        //оставляю как и было
        ...
        getPropertyRowNode: function(property, propsItemsContainer, disabled)
        {
            ...
            else
            {
                ...
                if(property.getId() == 48 || property.getId() == 47 || property.getId() == 2 || property.getId() == 3){
                    BX.addClass(propsItemNode, "col-md-6");
                }else if(property.getId() == 26 || property.getId() == 74 || property.getId() == 75 || property.getId() == 4){
                    BX.addClass(propsItemNode, "col-md-3");
                }else if(property.getId() == 1 || property.getId() == 40 || property.getId() == 22 || property.getId() == 23 || property.getId() == 24 || property.getId() == 76 || property.getId() == 77 || property.getId() == 78 || property.getId() == 79){
        ...
        editPropsComment: function(propsNode)
        {
            ...
            if(!BX("orderDescription")){
                this.deliveryBlockNode.after(propsCommentContainer);
            }
        },
