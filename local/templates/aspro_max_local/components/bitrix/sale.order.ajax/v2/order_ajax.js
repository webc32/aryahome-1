BX.namespace("BX.Sale.OrderAjaxComponent"), function () {
    ...
    BX.Sale && BX.Sale.Input && BX.Sale.Input.Utils && (BX.Sale.Input.Utils.asMultiple = function (value) {
        ...
        initializePrimaryFields: function () {
            ...
            this.initializePrimaryFields(), this.result = parameters.result || {}, this.prepareLocations(parameters.locations), this.params = parameters.params || {}, this.signedParamsString = parameters.signedParamsString || "", this.siteId = parameters.siteID || "", this.ajaxUrl = parameters.ajaxUrl || "", this.templateFolder = parameters.templateFolder || "", this.defaultBasketItemLogo = this.templateFolder + "/images/product_logo.png", this.defaultStoreLogo = this.templateFolder + "/images/pickup_logo.png", this.defaultDeliveryLogo = this.templateFolder + "/images/delivery_logo.png", this.defaultPaySystemLogo = this.templateFolder + "/images/pay_system_logo.png", this.orderBlockNode = BX(parameters.orderBlockId), this.totalBlockNode = BX(parameters.totalBlockId), this.mobileTotalBlockNode = BX(parameters.totalBlockId + "-mobile"), this.savedFilesBlockNode = BX("bx-soa-saved-files"), this.orderSaveBlockNode = BX("bx-soa-orderSave2"), this.mainErrorsNode = BX("bx-soa-main-notifications"), this.authBlockNode = BX(parameters.authBlockId), this.authHiddenBlockNode = BX(parameters.authBlockId + "-hidden"), this.basketBlockNode = BX(parameters.basketBlockId), this.basketBlockNode = {}, this.basketHiddenBlockNode = BX(parameters.basketBlockId + "-hidden"), this.regionBlockNode = BX(parameters.regionBlockId), this.regionBlockNode = {}, this.regionHiddenBlockNode = BX(parameters.regionBlockId + "-hidden"), this.paySystemBlockNode = BX(parameters.paySystemBlockId), this.paySystemHiddenBlockNode = BX(parameters.paySystemBlockId + "-hidden"), this.deliveryBlockNode = BX(parameters.deliveryBlockId), this.deliveryHiddenBlockNode = BX(parameters.deliveryBlockId + "-hidden"), this.pickUpBlockNode = BX(parameters.pickUpBlockId), this.pickUpBlockNode = {}, this.pickUpHiddenBlockNode = BX(parameters.pickUpBlockId + "-hidden"), this.propsBlockNode = BX(parameters.propsBlockId), this.propsHiddenBlockNode = BX(parameters.propsBlockId + "-hidden"), this.result.SHOW_AUTH && (this.authBlockNode.style.display = "", BX.addClass(this.authBlockNode, "bx-active"), this.authGenerateUser = "Y" !== this.result.AUTH.new_user_registration_email_confirmation && "Y" !== this.result.AUTH.new_user_phone_required), this.totalBlockNode && (this.totalInfoBlockNode = this.totalBlockNode.querySelector(".bx-soa-cart-total"), this.totalGhostBlockNode = this.totalBlockNode.querySelector(".bx-soa-cart-total-ghost")), this.options.deliveriesPerPage = parseInt(parameters.params.DELIVERIES_PER_PAGE), this.options.paySystemsPerPage = parseInt(parameters.params.PAY_SYSTEMS_PER_PAGE), this.options.pickUpsPerPage = parseInt(parameters.params.PICKUPS_PER_PAGE), this.options.showWarnings = !!parameters.showWarnings, this.options.propertyValidation = !!parameters.propertyValidation, this.options.priceDiffWithLastTime = !1, this.options.pickUpMap = parameters.pickUpMap, this.options.propertyMap = parameters.propertyMap, this.options.totalPriceChanged = !1, this.result.IS_AUTHORIZED && void 0 === this.result.LAST_ORDER_DATA.FAIL || this.initFirstSection(), this.initOrder = !0, this.initOptions(), this.editOrder(), this.bindEvents(), this.orderBlockNode.removeAttribute("style"), this.basketBlockScrollCheck(), "Y" === this.params.USE_ENHANCED_ECOMMERCE && this.setAnalyticsDataLayer("checkout"), "Y" === this.params.USER_CONSENT && this.initUserConsent()
            ...
            for (i in this.locationsInitialized = !0, this.fixLocationsStyle(this.deliveryBlockId, this.regionHiddenBlockNode), this.fixLocationsStyle(this.propsBlockNode, this.propsHiddenBlockNode), this.locations) {
                if (!this.locations.hasOwnProperty(i)) continue;
                if (!(locationNode = this.orderBlockNode.querySelector('div[data-property-id-row="' + i + '"]'))) continue;
                let value;
                clearButton = locationNode.querySelector("div.bx-ui-sls-clear"), inputStep = locationNode.querySelector("div.bx-ui-slst-pool"), inputSearch = locationNode.querySelector("input.bx-ui-sls-fake[type=text]"), locationNode.removeAttribute("style"), this.bindValidation(i, locationNode), clearButton && BX.bind(clearButton, "click", (function (e) {
                    var target = e.target || e.srcElement,
                        parent = BX.findParent(target, {tagName: "DIV", className: "form-group"}), locationInput;
                    parent && (locationInput = parent.querySelector("input.bx-ui-sls-fake[type=text]")), locationInput && BX.fireEvent(locationInput, "keyup")
                })), !this.firstLoad && this.options.propertyValidation && (inputStep && (arProperty = this.validation.properties[i], data = this.getValidationData(arProperty, locationNode), (section = BX.findParent(locationNode, {className: "bx-soa-section"})) && "true" == section.getAttribute("data-visited") && this.isValidProperty(data)), inputSearch && BX.fireEvent(inputSearch, "keyup")), inputSearch ? value = inputSearch.value : locationNode.querySelectorAll(".bx-ui-slst-input-block").forEach((function (node) {
                    value = node.querySelector(".bx-combobox-fake-as-input").textContent
                })), this.deliveryBlockNode.querySelectorAll(".destination_value").forEach((function (node) {
                    node.textContent = value
                }))
            }
            this.firstLoad && this.result.IS_AUTHORIZED && void 0 === this.result.LAST_ORDER_DATA.FAIL ? this.showActualBlock() : this.result.SHOW_AUTH || this.changeVisibleContent(), this.checkNotifications(), this.activeSectionId != this.propsBlockNode.id && this.editFadePropsContent(this.propsBlockNode.querySelector(".bx-soa-section-content"))
            ...
            if (zipProperty) {
                ...
                }), $zipWrapper = BX.create("DIV", {props: {className: "col-md-12"}, children: [$zipInner]});
                node.appendChild($zipWrapper), this.bindValidation(zipProperty.ID, propsItemNode)
            }
            ...
            this.result.IS_AUTHORIZED && !this.opened[this.deliveryBlockNode.id] && (this.opened[this.deliveryBlockNode.id] = !0, BX.removeClass(this.deliveryBlockNode, "bx-selected")), this.initialized.delivery || (this.opened[this.deliveryBlockNode.id] || this.deliveryBlockNode.classList.add("bx-step-completed"), (deliveryContent = node.querySelector(".bx-soa-section-content")) ? BX.cleanNode(deliveryContent) : (deliveryContent = this.getNewContainer(), node.appendChild(deliveryContent)), BX.cleanNode(node.querySelector(".bx-soa-section-location")), this.getDeliveryLocationInput(node.querySelector(".bx-soa-section-location")), this.getErrorContainer(deliveryContent), deliveryNode = BX.create("DIV", {props: {className: "bx-soa-pp row"}}), this.editDeliveryItems(deliveryNode), deliveryContent.appendChild(deliveryNode), this.editDeliveryInfo(deliveryNode), this.showPropInDelivery("ADDRESS", deliveryNode, BX.message("SOA_ADDRESS")), this.showPickUp(deliveryNode), this.getZipLocationInput(deliveryNode), "Y" !== this.params.HIDE_ORDER_DESCRIPTION && this.editPropsComment(deliveryNode), this.getBlockFooter(deliveryContent)), activeNodeMode && this.initialized.delivery || this.getDeliveryCompactInfo()
            ...
            var propsItemNode = BX.create("DIV"), textHtml = "", propertyType = property.getType() || "",
                propertyDesc = property.getDescription() || "", label;
            let bHasColumn = void 0 === with_column || with_column;
            switch (disabled ? propsItemNode.innerHTML = "<strong>" + BX.util.htmlspecialchars(property.getName()) + ":</strong> " : (BX.addClass(propsItemNode, "bx-soa-customer-field form-group"), bHasColumn && BX.addClass(propsItemNode, "col-sm-6"), textHtml += BX.util.htmlspecialchars(property.getName()), property.isRequired() && (textHtml += '<span class="bx-authform-starrequired">*</span> '), propertyDesc.length && "STRING" != propertyType && "NUMBER" != propertyType && "DATE" != propertyType && (textHtml += " <small>(" + BX.util.htmlspecialchars(propertyDesc) + ")</small>"), label = BX.create("LABEL", {
