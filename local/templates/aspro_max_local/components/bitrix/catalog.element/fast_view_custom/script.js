...
BX.extend(BasketButton, BX.PopupWindowButton);*/
  ...
  window.JCCatalogElementFast.prototype.ChangeInfo = function () {
    ...
    if (-1 < index) {
      ...
      if (!!this.offers[index].URL) {
        var arUrl = this.offers[index].URL.split("?");
        if (arUrl.length > 1) {
          var arUrl2 = $(".fast_view_frame").find(".form_head h2 a").attr("href").split("?");
          if (arUrl2.length > 1) {
            $(".fast_view_frame")
              .find(".form_head h2 a")
              .attr("href", $(".fast_view_frame").find(".form_head h2 a").attr("href").replace(arUrl2[1], arUrl[1]));
            $(".fast_view_frame")
              .find(".btn-wrapper a")
              .attr("href", $(".fast_view_frame").find(".btn-wrapper a").attr("href").replace(arUrl2[1], arUrl[1]));
          } else {
            $(".fast_view_frame").find(".form_head h2 a").attr("href", this.offers[index].URL);
            $(".fast_view_frame").find(".btn-wrapper a").attr("href", this.offers[index].URL);
          }
        }
      }

      var bSideChar = $(".properties__container_js").length;

      if (bSideChar) $(".properties__container_js").empty();
      ...
      if (this.config.showSkuProps) {
        ...
        if (!this.offers[index].DISPLAY_PROPERTIES || this.offers[index].DISPLAY_PROPERTIES.length === 0) {
          ...
          else $(this.obProduct).find(".properties__container_js").html(html);
        } else {
          for (var i in this.offers[index].DISPLAY_PROPERTIES) {
            ...
            if (display_type == "TABLE")
              ...
              html =
                ...
                "</div>" +
                "</div>";
              $(this.obProduct).find(".properties__container_js").append(html);
            }
          }
      ...
      $(this.obProduct).find(".quantity_block_wrapper .item-stock").html(this.offers[index].AVAILIABLE.HTML);
      ...
      if (arMaxOptions["THEME"]["CHANGE_TITLE_ITEM"] == "Y") {
        $(".fast_view_frame").find(".form_head h2 a").text(this.offers[index].NAME);
      }
    }
  };
  ...
  window.JCCatalogElementFast.prototype.SetSliderPict = function (obj, slider, config) {
    var container = $(".fastview-product__image .product-detail-gallery__slider.big"),
      containerThmb = $(".fastview-product__image .product-detail-gallery__slider.thmb"),
      slideHtml = "",
      slideThmbHtml = "";
      ...
      for (var i in slider) {
        if (typeof slider[i] == "object") {
          slideHtml +=
            ...
            '<a href="' +
            slider[i].BIG.src +
            '" data-fancybox="gallery_fast" class="product-detail-gallery__link fancy"><img class="product-detail-gallery__picture" border="0" src="' +
            slider[i].SMALL.src +
            '" alt="' +
            slider[i].ALT +
            '" title="' +
    ...
    if (obj.POPUP_VIDEO) {
      var popup_video =
        ...
        BX.message("POPUP_VIDEO") +
        "</span></a></div>";
      if ($(".fastview-product__top-info > .video-block").length)
        $(".fastview-product__top-info > .video-block").remove();
      $(popup_video).appendTo($(".fastview-product__top-info"));
      InitFancyBoxVideo();
    }
    ...
    /*var container=$('#fast_view_item .wrapp_thumbs'),
		slideHtml='';
  ...
  window.JCCatalogElementFast.prototype.setBuyBlock = function (th, obj) {
    ...
    if (obj.CONFIG.ACTION !== "NOTHING") {
      ...
      /*if(obj.CONFIG.ACTION == "ADD" && obj.CAN_BUY && obj.SHOW_ONE_CLICK_BUY!="N"){
			var ocb='<span class="btn btn-default white one_click transition_bg" data-offers="Y" data-item="'+obj.ID+'" data-iblockID="'+obj.IBLOCK_ID+'" data-quantity="'+obj.CONFIG.MIN_QUANTITY_BUY+'" data-props="'+obj.OFFER_PROPS+'" onclick="oneClickBuy('+obj.ID+', '+obj.IBLOCK_ID+', this)">'+
  ...
  window.JCCatalogElementFast.prototype.setOnlyPriceFromRange = function () {
    if (!!this.obPrice.price) {
      const product = $(this.obProduct);
      product.find(".js_price_wrapper > .price_matrix_wrapper").hide();
      this.setPriceMatrix({ setPriceMatrix: false });
    }
  };
  window.JCCatalogElementFast.prototype.setPriceMatrix = function (options) {
    var prices = "";
    if (!!this.obPrice.price) {
      var measure =
          this.offers[this.offerNum].MEASURE && this.offers[this.offerNum].SHOW_MEASURE == "Y"
      ...
      $(this.obProduct).find(".cost").addClass("sku_matrix");
      ...
      if (bSetPriceMatrix) {
        $(this.obProduct).find(".cost .js_price_wrapper").html(this.offers[this.offerNum].PRICE_MATRIX);
  ...
  window.JCCatalogElementFast.prototype.setPrice = function (obPrices, measure) {
    var prices = "";
    if (!!this.obPrice.price) {
      var measure =
          this.offers[this.offerNum].MEASURE && this.offers[this.offerNum].SHOW_MEASURE == "Y"
...
var sort_elem = $('.fast_view_button_cus').eq(1).find('span');
	sort_elem = sort_elem.data('sort-elems');
 

 container.addClass("blur");
	
	
	//передаем параметр чтобы пон€ть в файле ajax что страница поиска.
	data.pageSearch = request.indexOf('q=');
	...
	data.sort_elems = sort_elem;
	
	
	
	/*≈сли нет фильтра с ID товаров и это главна€ то запрашиваем его*/
	
	
	if(data.page_url == '/'){
		var filter_name = $('.CATALOG_TAB').find('.tabs_content .tab.cur').data('code');
		var filter = eval('filter_'+filter_name);
...
$('body').on('click', '.owl-prev, .owl-next', function () {
    var parrent = $(this).closest('.fastview-product__image_custom');
    var number_active = parrent.find('.owl-item.active .product-detail-gallery__item').data('number');
    parrent.find('.number_active').text(Number(number_active) + 1);
