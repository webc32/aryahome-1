...
if( $showCustomOffer && isset($arResult['OFFERS'][$arResult['OFFERS_SELECTED']]) ){
	...
	$arQuantityData = CMax::GetQuantityArray($totalCount, array('ID' => $arCurrentSKU["ID"]), 'N', ($arParams["USE_STORE"] == "Y" && $arResult["STORES_COUNT"] && $arResult['CATALOG_TYPE'] != CCatalogProduct::TYPE_SET));
} else {
		...
		<div class="fastview-product__image fastview-product__image_custom">
			<div class="product-detail-gallery product-detail-gallery--small js-notice-block__image">
				...
				<div class="product-detail-gallery__wrapper">
					...
					<?if($popupVideo):?>
						<div class="video-block video-block_popup_custom sm">
					...
					<div class="product-detail-gallery__slider product-detail-gallery__slider_custom_fast_view big<?if(!$bMagnifier):?> owl-carousel owl-theme big owl-bg-nav short-nav<?endif;?>" data-plugin-options='{"items": "1", "dots": false, "nav": true, "relatedTo": ".product-detail-gallery__slider.thmb", "loop": false, "autoWidth":true}'>
						<?if($popupVideo) {?>
							...
							$youtube = "https://www.youtube.com/oembed?url=https://www.youtube.com/watch?v=".$id_video."&format=json";
							...
							<div id="photo-0" data-number="0"class="product-detail-gallery__item product-detail-gallery__item--big text-center">
								<div class="preview_video" style="background-image:url(<?=$result['thumbnail_url'];?>)">
									<svg width="55" height="55" viewBox="0 0 55 55" fill="none" xmlns="http://www.w3.org/2000/svg">
										<circle cx="27.5" cy="27.5" r="27.5" fill="#d0a550"/>
										<path d="M38.0769 27.5L21.6827 39.1025L21.6827 15.8975L38.0769 27.5Z" fill="white"/>
									</svg>
								</div>
								<iframe id="iframe" class="frame_custom_fast_view product-detail-gallery__item product-detail-gallery__item--middle" allowfullscreen="allowfullscreen" src="<?=$popupVideo;?>?enablejsapi=1&mute=1&modestbranding=1&showinfo=0&controls=0&rel=0&rel=0&autohide=1" allow="autoplay" frameborder="0" scrolling="auto">
								</iframe>
							</div>
						<?}?>
						...
						?>
							<?foreach($arResult["MORE_PHOTO"] as $i => $arImage){
								...
								$title=$arImage["TITLE"];
								?>
								<div id="photo-<?=$i?>" data-number="<?=$i?>" class="product-detail-gallery__item product-detail-gallery__item--big text-center">
									<?if(!$isEmpty){?>
										<a href="<?=($viewImgType=="POPUP" ? $arImage["BIG"]["src"] : "javascript:void(0)");?>" <?=($bIsOneImage ? '' : 'data-fancybox="gallery_fast"')?> class="product-detail-gallery__link <?=($viewImgType=="POPUP" ? "popup_link fancy" : "line_link fancy_zoom");?>" title="<?=$title;?>">
			...
			<div class="slider_number">
				<span class="number_active"><?=($popupVideo ? '2' : '1')?></span>/<span class="count_slides"><?=$countPhoto?></span>
			</div>
		</div>
		<div class="fastview-product__info item_info">
			...
			<div class="prices_item_block">
				...
				<div class="middle_info1 main_item_wrapper">
					...
					<?if($arResult['OFFERS_CUSTOM']):?>
					<?$currentColor = ($_POST['product_color'] ? $_POST['product_color'] : $arResult['ID']);?>
					<?$currentSize = ($_POST['product_size'] ? $_POST['product_size'] : ($currentColor ? $currentColor : $arResult['ID']));?>
					<?
					// echo '<pre>';
					// var_dump($arResult['OFFERS_CUSTOM']);
					// echo '</pre>';
					?>
					...
					<div class="item-offers">
						...
						<div class="colors">
							<?foreach($arResult['OFFERS_CUSTOM'] as $color => $offer):?>
							<?
							if($_POST['offer_ajax']){
							...
							?>
							<span onclick="changeOfferFastView(this)" <?=$style?> data-url="<?=$offer['URL']?>" data-img="<?=$offer['SRC']?>" data-product_color="<?=$offer['ID']?>" <?=($currentColor == $offer['ID']) ? 'class="active"' : ''?>><img width="56px" height="56px" src="<?=$offer['SRC']?>" title="<?=$color?>"/></span>
							<?endforeach;?>
						</div>
						<?
						foreach($arResult['OFFERS_CUSTOM'] as $color => $offer){
							if($offer['ID'] == $currentColor){
								$sizes = $arResult['OFFERS_CUSTOM'][$color];
							}
						}
						?>
						<?if($sizes['SIZE']):?>
						...
						<div class="sizes">
							...
							<span onclick="changeOfferFastView(this)" <?=($currentSize == $size['ID']) ? 'class="active"' : ''?> data-url="<?=$size['URL']?>" data-product_size="<?=$size['ID']?>"><?=$size['RAZMER']?></span>
							<?endforeach;?>
						</div>
						<?endif;?>
								...
								<div class="js_price_wrapper">
									<?if($arCurrentSKU):?>
										...
										if(isset($arCurrentSKU['PRICE_MATRIX']) && $arCurrentSKU['PRICE_MATRIX'] && $arCurrentSKU['ITEM_PRICE_MODE'] == 'Q'): // USE_PRICE_COUNT?>
											...
											<?endif;?>
											<?if ($arParams['USE_PRICE_COUNT']):?>
												<?=showPriceMatrixCustom2($arCurrentSKU, $arParams, $strMeasure, $arAddToBasketData);?>
											<?endif;?>
										<?else:?>
											<?\Aspro\Functions\CAsproMaxItem::showItemPrices($arParams, $arCurrentSKU["PRICES"], $strMeasure, $min_price_id, ($arParams["SHOW_DISCOUNT_PERCENT_NUMBER"] == "Y" ? "N" : "Y"));?>
										<?endif;?>
					...
					<div class="buy_block">
						...
						<?if(!$arResult["OFFERS"]):?>
							...
							<div class="counter_wrapp list big clearfix">
								...
								<div onmousedown="try { rrApi.addToBasket('<?=$arResult["ID"]?>') } catch(e) {}" id="<? echo $arItemIDs["ALL_ITEM_IDS"]['BASKET_ACTIONS']; ?>" class="button_block basket_item_add <?=(($arAddToBasketData["ACTION"] == "ORDER" /*&& !$arResult["CAN_BUY"]*/) || !$arAddToBasketData["CAN_BUY"] || !$arAddToBasketData["OPTIONS"]["USE_PRODUCT_QUANTITY_DETAIL"] || ($arAddToBasketData["ACTION"] == "SUBSCRIBE" && $arResult["CATALOG_SUBSCRIBE"] == "Y")  ? "wide" : "");?>">
									<!--noindex-->
