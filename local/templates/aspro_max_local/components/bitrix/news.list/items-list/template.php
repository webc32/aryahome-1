			...
			<div class="tabs">
				<ul class="nav nav-tabs font_upper_md">
					...
					<?foreach($arResult['SECTIONS'] as $SID => $arSection):?>
						<?if(!$SID) continue;?>
						<li class="<?=$i++ == 0 ? 'active' : ''?> box-shadow rounded3 bordered"><a data-toggle="tab" href="#<?=$this->GetEditAreaId($arSection['ID'])?>"><?=$arSection['NAME']?></a></li>
					<?endforeach;?>
				</ul>
		<?endif;?>
					...
					<?foreach($arResult['SECTIONS'] as $SID => $arSection):?>
						...
						<div id="<?=$this->GetEditAreaId($arSection['ID'])?>" class="tab-pane <?=(!$si++ || !$arSection['ID'] ? 'active' : '')?>">
							...
							<?if($arParams['VIEW_TYPE'] !== 'accordion'):?>
								<div class="row sid items <?=($arParams['VIEW_TYPE'] == 'table' ? 'flexbox' : '')?>">
							<?endif;?>
