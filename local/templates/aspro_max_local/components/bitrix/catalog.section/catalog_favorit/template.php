...
<?if($arResult["ITEMS"]){?>
	...
	<div class="content_wrapper_block <?=$templateName;?>">
		...
		<?foreach($arResult["ITEMS"] as $key => $arItem){?>
			...
			$arQuantityData = CMax::GetQuantityArray($totalCount, array('ID' => $item_id), 'N', true);
			...
			if($bUseSkuProps)
			{
				...
				elseif($arItem["OFFERS"])
				{
					...
					$arQuantityData = CMax::GetQuantityArray($totalCount, $arItemIDs["ALL_ITEM_IDS"], 'N',true);
					...
					<div class="inner_wrap row">
						<div class="image col-md-6 text-center">
							<div class="image-wrapper image_wrapper_block js-notice-block__image flexbox">
								...
								<?showDelayCompareBtnFastView($arParams, $arItem, $arAddToBasketData, $totalCount, $bUseSkuProps, 'block lg', $bShowFastViewIcon, true, '_small');?>
							</div>
						</div>
