...
<? if ($arResult["ITEMS"]) : ?>
	...
	/*Массив сортировки передаваемый в попап с быстрым просмотром*/

	$sort_elems = [
		strtolower($arParams['ELEMENT_SORT_FIELD']) => 	strtolower($arParams['ELEMENT_SORT_ORDER']),	
			...
			<? foreach ($arResult["ITEMS"] as $arItem) { ?>
				...
				<? if ($arItem['OFFERS_CUSTOM']) : ?>
					<? if (!$arItem['NOT_CUSTOM_OFFERS']) : ?>
						...
						<div class="item-offers hideDesctop hide"><?/*hide*/?>
							<span class="offer_title 9999">Цвета:</span>
				...
				$resSect = CIBlockSection::GetList(
					array("SORT"=>"ASC"),
					array("ID" => $arItem['IBLOCK_SECTION_ID']),
					false,
					array("CODE"),
					false
				);
				...
				<div class="item-title">
					<a href="/catalog/<?=$codeSect["CODE"]?>/<?=$arItem["CODE"]?>/" class="<?= $bBigBlock && $bFonImg ? '' : 'dark_link' ?> js-notice-block__title option-font-bold font_sm"><span><?= $elementName; ?></span></a>
				</div>
						...
						<? else : ?>
							...
							?>
								<? if (\CMax::GetFrontParametrValue('SHOW_POPUP_PRICE') == 'Y' || $arItem['ITEM_PRICE_MODE'] == 'Q' || (\CMax::GetFrontParametrValue('SHOW_POPUP_PRICE') != 'Y' && $arItem['ITEM_PRICE_MODE'] != 'Q' && count($arItem['PRICE_MATRIX']['COLS']) <= 1)) : ?>
									...
									<? endif; ?><?
													$min_price = 0;
													foreach ($arItem['PRICE_MATRIX']['MATRIX'] as $key => $value) {
														if ($min_price == 0) {
															$min_price = $value["ZERO-INF"]["DISCOUNT_PRICE"];
														} else {
															if ($min_price > $value["ZERO-INF"]["DISCOUNT_PRICE"]) {
																$min_price = $value["ZERO-INF"]["DISCOUNT_PRICE"];
															}
														}
													}
									...
									<? if (count($arItem['PRICE_MATRIX']['ROWS']) > 1 || count($arItem['PRICE_MATRIX']['COLS']) > 1) : ?>
										<? if (count($arItem['PRICE_MATRIX']['CAN_BUY']) == 2 && in_array('7', $arItem['PRICE_MATRIX']['CAN_BUY']) && in_array('8', $arItem['PRICE_MATRIX']['CAN_BUY'])) : ?>
				...
				<div class="col-lg-<?= $col; ?><? if ($bBigBlock) : ?> col-md-8 col-sm-12 col-xs-12<? else : ?> col-md-4 col-sm-6 col-xs-6<? endif; ?> col-xxs-12 item item-parent catalog-block-view__item js-notice-block item_block<?= ($bBigBlock ? ' big' : ''); ?> <?= ($arParams['SET_LINE_ELEMENT_COUNT'] ? 'custom-line' : ''); ?> <?= $bComplect ? 'complect_item' : '' ?>" data-id="<?= $arItem["ID"] ?>" data-product_type="<?= $arItem["CATALOG_TYPE"] ?>">
					...
					<div class="catalog_item_wrapp catalog_item item_wrap main_item_wrapper<?= ($bBigBlock ? ' big' : ''); ?> <?= ($bFonImg ? '' : ' product_image') ?> <?= (($arParams['SHOW_PROPS'] == 'Y' && !$bBigBlock) || !$arItem['NOT_CUSTOM_OFFERS'] ? ' with-props' : ''); ?> <?= ($arItem["OFFERS"] ? 'has-sku' : '') ?>" id="<?= $arItem["strMainID"] ?>">
						<div class="inner_wrap  <?= $arParams["TYPE_VIEW_BASKET_BTN"] ?>">
							...
							<? endif; ?>
							<?$arItem['DETAIL_PAGE_URL'] ='/catalog/'.$codeSect["CODE"].'/'.$arItem["CODE"].'/'?>
							<? if ($arParams['SHOW_GALLERY'] == 'Y' && $arItem['OFFERS']) : ?>
								<div class="js-item-gallery hidden">
									<? \Aspro\Functions\CAsproMaxItem::showSectionGallery(array('ITEM' => $arItem, 'RESIZE' => $arResult['CUSTOM_RESIZE_OPTIONS'])); ?></div>
							<? endif; ?>
							<? if ($bBigBlock) : ?>
								<? if ($bFonImg) : ?>
							...
							<div class="image_wrapper_block js-notice-block__image <?= (($arParams['SHOW_PROPS'] == 'Y' && !$bBigBlock) || !$arItem['NOT_CUSTOM_OFFERS'] ? ' with-props' : ''); ?>">
								...
								<? if ($arParams['TYPE_VIEW_BASKET_BTN'] == 'TYPE_3') : ?>
									<? if (!$bFastViewFull) : ?><div class="like_icons block ce_cmp_hidden"><? endif; ?>
										<? if (!$bComplect) : ?>
											...
											<div class="fast_view_button 1 <?= $bFastViewFull ? 'fast_view_button--full fast_view_button--center' : '' ?>">
												<span title="<?= $fast_view_text ?>" class="rounded3 <?= $bFastViewFull ? 'font_upper_xs' : 'colored_theme_hover_bg' ?> " 
													
												data-sort-elems="<?=$sort_elems?>"	data-event="jqm" data-param-form_id="fast_view" data-param-iblock_id="<?= $arParams["IBLOCK_ID"]; ?>" data-param-id="<?= $arItem["ID"]; ?>" data-param-item_href="<?= urlencode($arItem["DETAIL_PAGE_URL"]); ?>" data-name="fast_view"><?= \CMax::showIconSvg($bFastViewFull ? "fw " : "fw ncolor colored", SITE_TEMPLATE_PATH . "/images/svg/quickview" . $typeSvg . ".svg"); ?><?= $bFastViewFull ? $fast_view_text : '' ?></span>
											</div>
										<? endif; ?>
										<? if (!$bFastViewFull) : ?>
										</div><? endif; ?>
								...
								<? else : ?>
									<? if ($bFastViewFull && $arParams['USE_FAST_VIEW'] != 'N' && !$bComplect) : ?>
										<div class="fast_view_button 2 fast_view_button--full">
									...
									<? endif; ?>
									<? if ($arParams['SHOW_FAST_VIEW_BTN_SEPARATELY'] == 'Y') : ?>
										<div class="btns-cont-without-fast-view">
											<div class="fast_view_button 3 fast_view_button--full fast_view_button_cus">
											<?$arItem['DETAIL_PAGE_URL'] ='/catalog/'.$codeSect["CODE"].'/'.$arItem["CODE"].'/'?>
												<span title="<?= $fast_view_text ?>" class="rounded2 font_upper_xs" data-sort-elems="<?=strtolower($arParams['ELEMENT_SORT_FIELD'])?>:<?=strtolower($arParams['ELEMENT_SORT_ORDER'])?>,<?=strtolower($arParams['ELEMENT_SORT_FIELD2'])?>:<?=strtolower($arParams['ELEMENT_SORT_ORDER2'])?>" data-event="jqm" data-param-form_id="fast_view" data-param-iblock_id="<?= $arParams["IBLOCK_ID"]; ?>" data-param-id="<?= $arItem["ID"]; ?>" data-param-item_href="<?= urlencode($arItem["DETAIL_PAGE_URL"]); ?>" data-name="fast_view"><?= \CMax::showIconSvg("fw ", SITE_TEMPLATE_PATH . "/images/svg/quickview" . $typeSvg . ".svg"); ?><?= $fast_view_text ?></span>
											</div>
								...
								<? if (!$bBigBlock) : ?>
									<? if (($arParams['SHOW_PROPS'] == 'Y' || !$arItem['NOT_CUSTOM_OFFERS']) && false) : ?>
													...
													<div class="properties__item">
														<div class="properties__title font_sxs muted">
															...
															<? if ($arProp["HINT"] && $arParams["SHOW_HINTS"] == "Y") : ?>
																<div class="hint"><span class="icon colored_theme_hover_bg"><i>?</i></span>
									...
									<? else : ?>
										...
										showImgCustom($arParams, $arItem, false);
										?>
									<? endif; ?>
								<? else : ?>
									<? if ($bFonImg) : ?>
	...
	dataLayer.push({  
		'ecommerce': {  
			...
			{  
				...
				'brand': "<?=$arItem['PROPERTY']["BRAND"]['VALUE']?>",  
				// 'category': 'Категория 1/Подкатегория 11',  
				'list': 'Catalog',  
				'position': <?=$id?>  
			},
			<?endforeach;?>
...
<script>
	$(document).ready(function(){
		$('.image_wrapper_block.js-notice-block__image > a').on('click',function(e){
			e.preventDefault();
			window.dataLayer = window.dataLayer || [];  
			dataLayer.push({  
				'ecommerce': {  
					...
					'click': {  
						...
						'products': [{  
							'name': $(this).closest('.item_block').find('.item-title span').text(),  
							'id': $(this).closest('.item_block').data('id'),  
							'price': $(this).closest('.item_block').find('.price.font-bold').data('value'),  
							// 'brand': 'Бренд 1',  
							// 'category': 'Категория 1/Подкатегория 11',  
							// 'position': 1  
						}]  
