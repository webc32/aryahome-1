...
foreach ($arResult['ITEMS'] as $key => $arItem) {
	if(array_key_exists($arItem['PROPERTIES']['NAIMENOVANIE_DLYA_SAYTA']["VALUE"], $arSku) && $arItem['PROPERTIES']['NAIMENOVANIE_DLYA_SAYTA']["VALUE"]){
			//if($availabelSku[$arItem['PROPERTIES']['NAIMENOVANIE_DLYA_SAYTA']["VALUE"]]['COLOR'] > 1 || $availabelSku[$arItem['PROPERTIES']['NAIMENOVANIE_DLYA_SAYTA']["VALUE"]]['SIZE'] > 1){
				$arResult['ITEMS'][$key]['OFFERS_CUSTOM'] = $arSku[$arItem['PROPERTIES']['NAIMENOVANIE_DLYA_SAYTA']["VALUE"]];
			//}else{
			//	$arResult['ITEMS'][$key]['NOT_CUSTOM_OFFERS'] = "Y";
			//}
		
	}
}
// echo '<pre>';
// var_dump($arResult['ITEMS'][$key]['OFFERS_CUSTOM']);
// echo '</pre>';
// die();
