...
foreach ($this->basketItems as $key => $row)
{
    ...
    $rsStore = CCatalogStoreProduct::GetList(
        array(),
        array('PRODUCT_ID' => $row['PRODUCT_ID'],'STORE_ID' => 3),
        false,
        false,
        array('AMOUNT')
    );
    if ($arStore = $rsStore->Fetch()) {
        $this->basketItems[$key]['AVAILABLE_QUANTITY'] = $arStore['AMOUNT'];
        // if($arStore['AMOUNT'] == 0){
        //     $arFields = Array(
...
foreach ($this->basketItems as $key => $row) {
    ...
    $arFilter = Array(
        "IBLOCK_ID" => CIBlockElement::GetIBlockByID($this->basketItems[$key]['PRODUCT_ID']),
        "ID" => $this->basketItems[$key]['PRODUCT_ID'],
    );
    ...
    if($nameForSite){
        ...
        while($ob = $res->GetNextElement()){
            ...
            if(!$this->basketItems[$key]['CUSTOM_SKU'][$arFields["PROPERTY_TSVET_VALUE"]]){
                $this->basketItems[$key]['CUSTOM_SKU'][$arFields["PROPERTY_TSVET_VALUE"]] = array(
...
foreach ($this->basketItems as $row) {
    ...
    if (!$row['DISCOUNT_PRICE']) {
        ...
        if ($ar_res = $db_res->Fetch()) {
            ...
            $oldPriceFull = $row['QUANTITY'] * $oldPrice;

            $raznicaByOldBrice = $oldPriceFull - ($row['PRICE'] * $row['QUANTITY']);

            $skidkaByOldPrice = (($row['PRICE'] - $oldPrice) / $oldPrice) * 100;

            $skidkaByOldPrice = round($skidkaByOldPrice);
            $skidkaByOldPrice = floor($skidkaByOldPrice);
            if ($raznicaByOldBrice > 0){
                $raznicaByOldBriceFormated = CurrencyFormat($raznicaByOldBrice, $row['CURRENCY']);
            }else{
                $raznicaByOldBriceFormated = false;
            }

            $sumPriceWithoutDicsount += $oldPriceFull;

            $oldPriceFull = CurrencyFormat($oldPriceFull, $row['CURRENCY']);
            ...
            if ($skidkaByOldPrice) {
                $showDicsountByOldPrice = true;
            }
        }

    } else {
        $skidka = (($row['PRICE'] - $row['FULL_PRICE']) / $row['FULL_PRICE']) * 100;
        $skidka = round($skidka);
        $skidka = floor($skidka);

        $sumPriceWithoutDicsount += $row['SUM_FULL_PRICE'];

    }

    $totalRaznica += $row['DISCOUNT_PRICE'] * $row['QUANTITY'];
    ...
    $rowData = array(
        ...
        'SHOW_DICSOUNT_BY_OLD_PRICE' => $showDicsountByOldPrice,
        'SKIDKDA_PERCENT_BY_OLD_PRICE' => $skidkaByOldPrice,
        'PRICE' => $row['PRICE'],
        ...
        'PRICE_OLD' => $oldPrice,
        'SUM_FUL_PRICE_OLD' => $oldPriceFull,
        'RAZNICA_PRICE_BY_OLD_PRICE' => $raznicaByOldBriceFormated,
        'COLUMN_LIST' => array(),
        'SHOW_LABEL' => false,
        'LABEL_VALUES' => array(),
        ...
        'CUSTOM_OFFERS' => $row['CUSTOM_OFFERS']
    );
        ...
        while ($arProp = $rsProps->Fetch()) {
            if ($arProp['VALUE_ENUM']) {
                ...
                $arProps[] = [
                    ...
                    'CLASS' => 'sticker_' . strtolower($arProp['CODE']),
                ];
            }
        }
    }
...
if ($totalRaznica > 0) {
    $totalData['HAS_RAZNICA'] = true;
    $totalData['TOTAL_RAZNICA_FORMATED'] = CurrencyFormat($totalRaznica, $row['CURRENCY']);
    ...
    if (abs($totalSkidka) > 0){
        $totalData['TOTAL_SKIDKA'] = $totalSkidka;
        $totalData['TOTAL_SKIDKA_FORMATED'] = '-' . $totalSkidka . '%';
    }
}
