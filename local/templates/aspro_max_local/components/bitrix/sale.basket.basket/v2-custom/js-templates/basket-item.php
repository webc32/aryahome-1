...
<script id="basket-item-template" type="text/html">
	<tr class="basket-items-list-item-wrapper{{#SHOW_RESTORE}} basket-items-list-item-container-expend{{/SHOW_RESTORE}} {{SERVICES_CLASS}} {{WITH_SERVICES_CLASS}} basket-product" id="basket-item-{{ID}}" data-entity="basket-item" data-id="{{ID}}">
		{{#SHOW_RESTORE}}
		...
		{{^SHOW_RESTORE}}
				...
				<td class="basket-items-list-item-picture" {{#HAS_SERVICES}}rowspan="2"{{/HAS_SERVICES}}>
					<div class="basket-item-block-image<?=(!isset($mobileColumns['PREVIEW_PICTURE']) ? ' hidden-xs' : '')?>">
						<div class="fast_view"  data-event="jqm" data-param-form_id="fast_view" data-param-iblock_id="{{IBLOCK_ID}}" data-param-id="{{PRODUCT_ID}}" data-param-item_href="{{DETAIL_PAGE_URL}}" data-name="fast_view">
							<?=\CMax::showIconSvg("fw", SITE_TEMPLATE_PATH."/images/svg/quickview.svg");?>
						</div>
						{{#DETAIL_PAGE_URL}}
							...
							foreach ($arParams['PRODUCT_BLOCKS_ORDER'] as $blockName)
							{
								switch (trim((string)$blockName))
								{
									...
									case 'custom_sku':
											?>
												{{#CUSTOM_OFFERS}}
												<div class="basket-item-property basket-item-property-scu-image basket-item-property-scu-color"
													data-entity="basket-item-sku-block-color">
												...
												<div class="basket-item-property basket-item-property-scu-image basket-item-property-scu-size" data-entity="basket-item-sku-block-size">
													...
													<div class="basket-item-property-value">
														<ul class="basket-item-scu-list">
															...
															onclick="refreshSku(this)"
															>
															<span class="basket-item-scu-item-inner">{{RAZMER}}</span>
														</li>
														{{/SIZE}}
														{{/SELECTED}}
														{{/CUSTOM_SKU}}
			...
			<?
			if ($useSumColumn || $useActionColumn)
			{
				?>
				<td class="basket-items-list-item-price<?=(!isset($mobileColumns['SUM']) ? ' hidden-xs' : '')?>">
					<?if ($useSumColumn){?>
						<div class="basket-item-price">
                            ...
                            {{^SHOW_DISCOUNT_PRICE}}
                                {{#SHOW_DICSOUNT_BY_OLD_PRICE}}
                                <div class="discount-by-old-price">
                                        <span class="discount-by-old-price__val" id="discount-by-old-price-{{ID}}">
                                            {{{SUM_FUL_PRICE_OLD}}}
                                        </span>
<!--                                    <span class="discount-by-old-price__dicsount">{{DISCOUNT_BY_OLD_PRICE}}%</span>-->
                                </div>
                                {{/SHOW_DICSOUNT_BY_OLD_PRICE}}

                                {{#RAZNICA_PRICE_BY_OLD_PRICE}}

                                    <div class="basket-item-price-difference custom">
                                        {{#SKIDKDA_PERCENT_BY_OLD_PRICE}}
                                            <div class="sale-number rounded2">
                                                <div class="value">{{{SKIDKDA_PERCENT_BY_OLD_PRICE}}}%</div>
                                            </div>
                                        {{/SKIDKDA_PERCENT_BY_OLD_PRICE}}
                                        <div class="basket-item-price-difference__sum-economy 1">
							...
							{{#SHOW_DISCOUNT_PRICE}}
								...
								<div class="basket-item-price-difference custom">
                                    {{#DISCOUNT_PRICE_PERCENT}}
                                    <div class="sale-number rounded2">
                                        <div class="value">{{{DISCOUNT_PRICE_PERCENT}}}%</div>
                                    </div>
                                    {{/DISCOUNT_PRICE_PERCENT}}
                                    <div class="basket-item-price-difference__sum-economy 2">
                                        <?=Loc::getMessage('SBB_BASKET_ITEM_ECONOMY')?>
                                        <span id="basket-item-sum-price-difference-{{ID}}" style="white-space: nowrap;">
