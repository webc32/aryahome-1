...
<? endforeach; ?>
    ...
    <div class="basket_wrap custom">
            ...
            <div class="items">
                <? foreach ($arResult["GRID"]["ROWS"] as $k => $arItem):
                    ...
                    if ($arItem["DELAY"] == "N" && $arItem["CAN_BUY"] == "Y"):
                        ...
                        <div class="item <?= ($isServices ? 'hidden' : '') ?>" data-id="<?= $arItem["ID"] ?>"
                             product-id="<?= $arItem["PRODUCT_ID"] ?>" data-iblockid="<?= $arItem["IBLOCK_ID"] ?>"
                             <? if ($arItem["QUANTITY"] > $arItem["AVAILABLE_QUANTITY"]): ?>data-error="no_amounth"<? endif;
                        ?> <? if ($isServices): ?>data-parent_product_id="<?= $idParentProduct ?>"<? endif;
                        ?>>
                                ...
                                <div class="image">
                                    <? if (strlen($arItem["PREVIEW_PICTURE"]["SRC"]) > 0) {
                                        ...
                                        <img src="<?= $arItem["PREVIEW_PICTURE"]["SRC"] ?>"
                                             alt="<?= (is_array($arItem["PREVIEW_PICTURE"]["ALT"]) ? $arItem["PREVIEW_PICTURE"]["ALT"] : $arItem["NAME"]); ?>"
                                             title="<?= (is_array($arItem["PREVIEW_PICTURE"]["TITLE"]) ? $arItem["PREVIEW_PICTURE"]["TITLE"] : $arItem["NAME"]); ?>"/>
                                        <? if (strlen($arItem["DETAIL_PAGE_URL"]) > 0):?></a><? endif; ?>
                                    <? } elseif (strlen($arItem["DETAIL_PICTURE"]["SRC"]) > 0) {
                                        ?>
                                        <? if (strlen($arItem["DETAIL_PAGE_URL"]) > 0):?><a href="<?= $arItem["DETAIL_PAGE_URL"] ?>" class="thumb"><? endif; ?>
                                        <img src="<?= $arItem["DETAIL_PICTURE"]["SRC"] ?>"
                                             alt="<?= (is_array($arItem["DETAIL_PICTURE"]["ALT"]) ? $arItem["DETAIL_PICTURE"]["ALT"] : $arItem["NAME"]); ?>"
                                             title="<?= (is_array($arItem["DETAIL_PICTURE"]["TITLE"]) ? $arItem["DETAIL_PICTURE"]["TITLE"] : $arItem["NAME"]); ?>"/>
                                        <? if (strlen($arItem["DETAIL_PAGE_URL"]) > 0):?></a><? endif; ?>
                                    <? } else {
                                        ?>
                                        <? if (strlen($arItem["DETAIL_PAGE_URL"]) > 0):?><a href="<?= $arItem["DETAIL_PAGE_URL"] ?>" class="thumb"><? endif; ?>
                                        <img src="<?= SITE_TEMPLATE_PATH ?>/images/svg/noimage_product.svg"
                                             alt="<?= $arItem["NAME"] ?>" title="<?= $arItem["NAME"] ?>" width="70"
                                             height="70"/>
                                        <? if (strlen($arItem["DETAIL_PAGE_URL"]) > 0):?></a><? endif; ?>
                                    <? } ?>
                                    <? if (!empty($arItem["BRAND"])):?>
                                        <div class="ordercart_brand"><img src="<?= $arItem["BRAND"] ?>"/>
                                        </div><? endif; ?>
                                </div>
                                <div class="body-info">
                                    <div class="description">
                                        <div class="name">
                                            <? if (strlen($arItem["DETAIL_PAGE_URL"]) > 0): ?><a
                                                ...
                                                ?><?= $arItem["NAME"] ?><? if (strlen($arItem["DETAIL_PAGE_URL"]) > 0): ?></a><? endif;
                                        ?>
                                        </div>
                                        <?if($arItem['CUSTOM_SKU']):?>
                                        <div class="basket-item-property basket-item-property-scu-image-color" data-entity="basket-item-sku-block-color">
                                            <div class="basket-item-property-name">Цвет</div>
                                            <div class="basket-item-property-value">
                                                <ul class="basket-item-scu-list">
                                                    <?foreach($arItem['CUSTOM_SKU'] as $skuId => $skuValue):?>
                                                    <li class="basket-item-scu-item <?=$skuValue['SELECTED'] ? 'selected' : ''?>" title="<?=$skuValue['NAME_COLOR']?>" data-entity="basket-item-sku-field" data-initial="<?=$skuValue['SELECTED'] ? true : false?>" data-value-id="<?=$skuValue['PRODUCT_ID']?>" <?if(!$skuValue['SELECTED']):?>onclick="refreshSku(this)"<?endif;?> >
                                                        <span class="basket-item-scu-item-inner-color"
                                                        style="background-image: url(<?=$skuValue['SRC']?>);"></span>
                                                    </li>
                                                    <?endforeach;?>
                                                </ul>
                                            </div>
                                        ...
                                        <div class="basket-item-property basket-item-property-scu-image-size" data-entity="basket-item-sku-block-size">
                                            <div class="basket-item-property-name">Размер</div>
                                            <div class="basket-item-property-value">
                                                <ul class="basket-item-scu-list">
                                                    <?foreach($arItem['CUSTOM_SKU'] as $skuId => $skuValue):?>
                                                    <?if($skuValue['SELECTED'] && $skuValue['SIZE']):?>
                                                        <?foreach($skuValue['SIZE'] as $size):?>
                                                        <li class="basket-item-scu-item <?=$size['SELECTED'] ? 'selected' : ''?>" title="<?=$size['RAZMER']?>" data-entity="basket-item-sku-field" data-initial="<?=$size['SELECTED'] ? true : false?>}" data-value-id="<?=$size['PRODUCT_ID']?>" <?if(!$size['SELECTED']):?>onclick="refreshSku(this)" <?endif;?>>
                                                            <span class="basket-item-scu-item-inner-size"><?=$size['RAZMER']?></span>
                                                        </li>
                                                        <?endforeach;?>
                                                    <?endif;?>
                                                    <?endforeach;?>
                                    ...
                                    <div class="bottom">
                                            ...
                                            <div class="cost prices clearfix">
                                                ...
                                                <? if (doubleval($arItem["DISCOUNT_PRICE_PERCENT"]) > 0 && $bDiscountColumn) {
                                                    ...
                                                    <div class="price discount">
                                                        <span><?= $arItem["FULL_PRICE_FORMATED"] ?></span></div>

                                                    <input type="hidden" name="item_price_<?= $arItem["ID"] ?>"
                                                           value="<?= $arItem["PRICE"] ?>"/>
                                                    <input type="hidden" name="item_price_discount_<?= $arItem["ID"] ?>"
                                                ...
                                                <?/* if (strlen($arItem["NOTES"]) > 0 && $bTypeColumn):?>
                                                    <?
                                                    if ($arItem['PRICE_TYPE_ID'] == '8') $arItem["NOTES"] = 'Цена';
                                                    ?>
                                                    <div class="price_name"><?= $arItem["NOTES"] ?></div>
                                                <? endif; */?>
                                                <input type="hidden" name="item_summ_<?= $arItem["ID"] ?>"
                                        ...
                                        <div class="buy_block">
                                            <div class="counter_block basket">
                                                ...
                                                ?>
                                                <? if (isset($arItem["AVAILABLE_QUANTITY"]) /*&& floatval($arItem["AVAILABLE_QUANTITY"]) != 0*/ /*&& !CSaleBasketHelper::isSetParent($arItem)*/):?>
                                                    <span
                                                    onclick="setQuantityFly('<?= $arItem["ID"] ?>', '<?= $arItem["MEASURE_RATIO"] ?>', 'down')"
                                                    class="minus"></span><? endif; ?>
                                                <input
                                                        type="text"
                                                        class="text"
                                                ...
                                                <? if (isset($arItem["AVAILABLE_QUANTITY"]) /*&& floatval($arItem["AVAILABLE_QUANTITY"]) != 0*/ /*&& !CSaleBasketHelper::isSetParent($arItem)*/):?>
                                                    <span
                                                    onclick="setQuantityFly('<?= $arItem["ID"] ?>', '<?= $arItem["MEASURE_RATIO"] ?>', 'up')"
                                                    class="plus"></span><? endif; ?>
                                            </div>
                                            <input type="hidden" id="QUANTITY_<?= $arItem['ID'] ?>"
                                                   name="QUANTITY_<?= $arItem['ID'] ?>"
                                                   value="<?= $arItem["QUANTITY"] ?>"/>
                                            <? if ($arItem["QUANTITY"] > $arItem["AVAILABLE_QUANTITY"]):?>
                                                <div class="error"><?= GetMessage("NO_NEED_AMMOUNT") ?></div><? endif; ?>
                                        </div>
                                        <div class="summ">
                                            ...
                                            <?if (doubleval($arItem["DISCOUNT_PRICE_PERCENT"]) > 0 && $bDiscountColumn):?>
                                                <?
                                                  $hasSomeDiscount = true;
                                                  $percentSkida = $arItem['PERCENT_SKIDKA'].'%';
                                                  $raznica = $arItem['RAZNICA'];
                                                ?>
                                            <?endif;?>
                                            <? if (doubleval($arItem["DISCOUNT_PRICE_PERCENT"]) == 0 && abs($arItem['SKIDKDA_PERCENT_BY_OLD_PRICE']) > 0):?>
                                                <?


                                                $percentSkida = $arItem['SKIDKDA_PERCENT_BY_OLD_PRICE'].'%';
                                                $raznica = $arItem['RAZNICA_BY_OLD_PRICE_FORMATED'];
                                                $hasSomeDiscount = true;
                                                ?>
                                            <? endif; ?>
                                            <?if ($hasSomeDiscount):?>
                                            <?$hasSomeDiscount = false;?>
                                            <div class="discount-by-old-price">
                                                    <span class="discount-by-old-price__val">
                                                        <div class="basket-item-price-difference custom">
                                                            <div class="sale-number rounded2">
                                                                <div class="value"><?=$percentSkida?></div>
                                                            </div>
                                                            <div class="basket-item-price-difference__sum-economy">
                                                                Экономия
                                                            <span id="basket-item-sum-price-difference-<?=$arItem['ID']?>" style="white-space: nowrap;">
                                                                <?=$raznica?>
                                                            </span>
                                                        </div>
                                                        </div>
                                            ...
                                            <div class="delay-cell delay">
                                                <a class="action_item"
                                                   href="<?= str_replace("#ID#", $arItem["ID"], $arUrls["delay"]) ?>">
                                                    <span class="icon"
                                                          title="<?= GetMessage("SALE_DELAY"); ?>"><?= CMax::showIconSvg("wish colored_theme_hover_text", SITE_TEMPLATE_PATH . '/images/svg/chosen_small.svg', '', '', true, false); ?></span>
                                                </a>
                                            </div>
                                        <? endif; ?>
                                    </div>
                                    <div class="services_in_basket buy_services_wrap"
                                         data-parent_product="<?= $arItem["PRODUCT_ID"] ?>">
                                        <? /*services*/
                                    ...
                                    <? if ($bDeleteColumn):?>
                                        <div class="remove-cell"><a class="remove"
                                                                    href="<?= str_replace("#ID#", $arItem["ID"], $arUrls["delete"]) ?>"
                                                                    title="<?= GetMessage("SALE_DELETE") ?>"><?= CMax::showIconSvg("remove colored_theme_hover_text", SITE_TEMPLATE_PATH . '/images/svg/catalog/cancelfilter.svg', '', '', true, false); ?></a>
                                        </div>
                                    <? endif; ?>
                                </div>
                            </div>
        ...
        if ($arResult['SUM_PRICE_WITHOUT_DISCOUNT'] > 0){
            ...
            <div class="total pull-right<?= ($arResult["DISCOUNT_PRICE_ALL"] ? ' w-discount' : ''); ?>">
                <? foreach ($arTotal as $key => $value):?>
                    <? if ($value["VALUES"] && $value["NAME"]):?>
                        <div class="item_title"><?= $value["NAME"] ?></div><? endif; ?>
                <? endforeach; ?>
                <?
//                echo '<pre>';
//                var_dump($arTotal);
                    ...
                    <? foreach ($arTotal as $key => $value):?>
                        <? if ($value["VALUES"] && $value["NAME"]):?>
                            ...
                            <? if ($key == "PRICE"):?>
                                ...
                                <? if ($arResult["DISCOUNT_PRICE_ALL"] || $arResult['SUM_PRICE_WITHOUT_DISCOUNT']):?>
                                    <div data-type="price_discount">
                                        <div class="price"><?= $value["VALUES"]["ALL"]; ?></div>
                                        <div class="price discount">
                                            <strike><?= $value["VALUES"]["WITHOUT_DISCOUNT"]; ?></strike></div>
                                    </div>
                                <? else:?>
                                    <div data-type="price_normal">
                                        <div class="price"><?= $arResult["allSum_FORMATED"]; ?></div>
                                    </div>
                                <? endif; ?>
                            <? elseif ($value["VALUE"]):?>
                                <div data-type="<?= strToLower($key) ?>">
                                    <div class="price"><?= $value["VALUE"] ?></div>
                                </div>
                            <? endif; ?>
                        <? endif; ?>
                    <? endforeach; ?>
                    <?if ($arResult['TOTAL_RAZNICA_FORMATED']):?>
                        <div class="basket-item-price-difference custom">
                            <div class="sale-number rounded2">
                                <div class="value"><?=$arResult['TOTAL_SKIDKA_FORMATED']?></div>
                            </div>
                            <div class="basket-item-price-difference__sum-economy">
                                Экономия
                                <span id="basket-item-sum-price-difference-796861" style="white-space: nowrap;">
                                    <?=$arResult['TOTAL_RAZNICA_FORMATED']?>
                                </span>
                            </div>
                        </div>
        ...
        <div class="buttons clearfix">
            <? if ($arParams["AJAX_MODE_CUSTOM"] != "Y"):?>
                <div class="basket_update pull-left">
                    <button type="submit" name="BasketRefresh" class="btn btn-default white grey refresh btn-lg">
                        <span><?= GetMessage("SALE_REFRESH") ?></span></button>
            ...
            <? if (!$arError["ERROR"]):?>
                <? if (\Bitrix\Main\Config\Option::get("aspro.max", "SHOW_ONECLICKBUY_ON_BASKET_PAGE", "N") == "Y"):?>
                    <div class="basket_fast_order pull-right">
                        <a onclick="oneClickBuyBasket()"
                           class="btn btn-default fast_order btn-lg"><span><?= GetMessage("SALE_FAST_ORDER") ?></span></a>
                        <div class="description"><?= GetMessage("SALE_FAST_ORDER_DESCRIPTION"); ?></div>
                    </div>
                <? else:?>
                    <div class="wrap_button pull-right">
                        <a href="<?= $arParams["PATH_TO_BASKET"] ?>"
                           class="btn btn-transparent-border-color btn-lg"><span><?= GetMessage("GO_TO_BASKET") ?></span></a>
            ...
            <? if ($arParams["SHOW_FULL_ORDER_BUTTON"] == "Y" && !$arError["ERROR"]):?>
                <div class="basket_checkout pull-right">
                    <a data-href="<?= $arParams["PATH_TO_ORDER"]; ?>" href="<?= $arParams["PATH_TO_ORDER"]; ?>"
                       class="btn btn-transparent-border-color checkout btn-lg"><span><?= GetMessage("SALE_ORDER") ?></span></a>
