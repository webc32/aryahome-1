	...
	<div class="menu top top-mobile-menu">
		<ul class="top">
			<?foreach($arResult as $arItem):?>
				...
				$changeColorClass = ((isset($arItem['PARAMS']['CLASS']) && strpos($arItem['PARAMS']['CLASS'], 'version_site_mobile') !== false) ? 'version_site_mobile' : '');
				?>
				<?$bParent = $arItem['CHILD'] && $bShowChilds;?>
				<li<?=($arItem['SELECTED'] ? ' class="selected"' : '')?> >
					<a class="dark-color<?=($bParent ? ' parent' : '')?> <?=$changeColorClass?>" href="<?=$arItem["LINK"]?>" title="<?=$arItem["TEXT"]?>">
											...
											<?if(is_array($arImg)):?>
												<span class="image top-mobile-menu__image colored_theme_svg">
													<?if(strpos($arImg["src"], ".svg") !== false):?>
														<?=CMax::showIconSvg("cat_icons", $arImg["src"]);?>
													<?else:?>
