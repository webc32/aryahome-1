...
if($arResult["ITEMS"]){?>
		...
		<div class="slide-block">
			...
			<div class="slide-block__body">
				<div class="bx_filter_section bordered rounded3">
					<form name="<?echo $arResult["FILTER_NAME"]."_form"?>" action="<?echo $arResult["FORM_ACTION"]?>" method="get" class="smartfilter" id="smartfilter<?=$arParams['AJAX_FILTER_FLAG'];?>">
						...
						<div class="bx_filter_parameters">
							...
							<?endforeach;?>
							<?if ($arParams["~htmlSections"] && $arParams["TOP_VERTICAL_FILTER_PANEL"] == "N"):?>
								...
								<div class="bx_filter_parameters_box bx_sort_filter active" data-expanded="Y" data-prop_code="SECT" data-property_id="SECT">
									<div class="bx_filter_parameters_box_title  colored_theme_hover_bg-block" >
										<div>
											<a class="dark_link" href="<?=$APPLICATION->GetCurPageParam('', $arDeleteParams)?>"><?=Loc::getMessage("FILTER_SECTON_SEARCH");?></a>
										</div>
										<?=CMax::showIconSvg("down colored_theme_hover_bg-el", SITE_TEMPLATE_PATH.'/images/svg/trianglearrow_down.svg', '', '', true, false);?>
									</div>
									<div class="bx_filter_block">
										<?=$arParams["~htmlSections"]?>
									</div>
								</div>
							<?endif;?>
							<?
							...
							foreach($arResult["ITEMS"] as $key => $arItem){
							    if(isset($arItem["ASPRO_FILTER_SORT"]) && $arItem["VALUES"]){
									...
									if($arItem["DISPLAY_EXPANDED"]=="Y"){
										$class="active colored_theme_bg_before";
									}
									$isFilter=true;
									?>
							...
							foreach($arResult["ITEMS"] as $key=>$arItem)
							{
								$key = $arItem["ENCODED_ID"].$arParams['AJAX_FILTER_FLAG'];
								if(isset($arItem["PRICE"])):
									...
									<div class="bx_filter_parameters_box active" data-property_name="<?=$arItem["NAME"]?>">
										<span data-f="<?=Loc::getMessage('CT_BCSF_SET_FILTER')?>" data-fi="<?=Loc::getMessage('CT_BCSF_SET_FILTER_TI')?>" data-fr="<?=Loc::getMessage('CT_BCSF_SET_FILTER_TR')?>" data-frm="<?=Loc::getMessage('CT_BCSF_SET_FILTER_TRM')?>" class="bx_filter_container_modef"></span>
										<div class="bx_filter_parameters_box_title colored_theme_hover_bg-block" >
											<div><?=(count($arParams['PRICE_CODE']) > 1 ? $arItem["NAME"] : Loc::getMessage("PRICE"));?></div>
											<?=CMax::showIconSvg("down colored_theme_hover_bg-el", SITE_TEMPLATE_PATH.'/images/svg/trianglearrow_down.svg', '', '', true, false);?>
										</div>
										<div class="bx_filter_block">
							...
							foreach($arResult["ITEMS"] as $key=>$arItem)
							{
								...
								<div class="bx_filter_parameters_box <?=$class;?>" data-expanded="<?=($arItem["DISPLAY_EXPANDED"] ? $arItem["DISPLAY_EXPANDED"] : "N");?>" data-prop_code=<?=strtolower($arItem["CODE"]);?> data-property_id="<?=$arItem["ID"]?>" data-property_name="<?=$arItem["NAME"]?>">
									...
									<div class="bx_filter_block <?=($arItem["CODE"] != "IN_STOCK" && $arItem["DISPLAY_TYPE"] != 'A' ? '' : '');?> <?=($arItem["CODE"] != "IN_STOCK" && $arItem["PROPERTY_TYPE"]!="N" && ($arItem["DISPLAY_TYPE"] != "P" && $arItem["DISPLAY_TYPE"] != "R") ? "limited_block scrollblock" : "");?>" <?=$style;?>>
										<div class="bx_filter_parameters_box_container <?=($arItem["DISPLAY_TYPE"]=="G" ? "pict_block" : "");?> ">
										<?
										$arCur = current($arItem["VALUES"]);
													...
													<div class="bx_filter_select_block" onclick="smartFilter.showDropDownPopup(this, '<?=CUtil::JSEscape($key)?>')">
														...
														<div class="input_wr_all">
															<input
																...
																value=""
																data-title="<? echo Loc::getMessage("CT_BCSF_FILTER_ALL"); ?>"
															/>
															<?foreach ($arItem["VALUES"] as $val => $ar):?>
																<input
																	style="display: none"
							...
							<div class="bx_filter_button_box btns ajax-btns">
								<div class="bx_filter_block">
									<div class="bx_filter_parameters_box_container">
										...
										<button class="bx_filter_search_reset btn btn-default btn-transparent<?=($bActiveFilter && $bActiveFilter[1] != 'clear' ? '' : ' hidden');?> pull-left" type="reset" id="del_filter" name="del_filter" data-href="" title="<?=Loc::getMessage("CT_BCSF_DEL_FILTER")?>">
											<?=CMax::showIconSvg("reset", SITE_TEMPLATE_PATH.'/images/svg/catalog/cleanfilters_big.svg', '', '', true, false);?>
										</button>
										<div class="filter-bnt-wrapper hidden"><button class="bx_filter_search_button btn btn-default btn-lg btn-transparent" type="button" id="set_filter1" name="set_filter1" data-href=""><?=Loc::getMessage("CT_BCSF_SET_FILTER")?></button></div>
									</div>
								</div>
							</div>
						<?endif;?>
	...
	<script>
		var smartFilter<?=$arParams['AJAX_FILTER_FLAG'];?> = new JCSmartFilter('<?echo CUtil::JSEscape($arResult["FORM_ACTION"])?>', '<?=$arParams["VIEW_MODE"];?>', <?=CUtil::PhpToJSObject($arResult["JS_FILTER_PARAMS"])?>);
		<?if(!$isFilter){?>
