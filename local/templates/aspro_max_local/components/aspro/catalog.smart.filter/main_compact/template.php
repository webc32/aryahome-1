		...
		<div class="bx_filter_section clearfix">
			<form name="<?echo $arResult["FILTER_NAME"]."_form"?>" action="<?echo $arResult["FORM_ACTION"]?>" method="get" class="smartfilter">
				<div class="bx_filter_parameters_box title">
					<div class="bx_filter_parameters_box_title filter_title <?=($bActiveFilter ? 'active-filter' : '')?>">
						<?=CMax::showIconSvg("icon", SITE_TEMPLATE_PATH.'/images/svg/catalog/filter.svg', '', '', true, false);?>
						<span class="font_upper_md font-bold"><?=Loc::getMessage("FILTER_TITLE_COMPACT");?></span>
					...
					foreach($arResult["ITEMS"] as $key=>$arItem)
					{
						...
						<div class="bx_filter_parameters_box prop_type_<?=$arItem["PROPERTY_TYPE"];?><?=(isset($arItem['PROPERTY_SET']) && $arItem['PROPERTY_SET'] == 'Y' ? ' set' : '');?>" data-prop_code="<?=strtolower($arItem["CODE"]);?>" data-property_id="<?=$arItem["ID"]?>">
							...
							<div class="bx_filter_block <?=($arItem["CODE"]=="IN_STOCK" ? "limited_block" : "");?>" <?=$style;?>>
								<div class="bx_filter_parameters_box_container <?=($arItem["DISPLAY_TYPE"]=="G" ? "pict_block" : "");?> <?=($arItem["CODE"] != "IN_STOCK" && $arItem["DISPLAY_TYPE"] != 'A' ? 'scrollblock' : '');?>">
								<?
				...
				<?if($isFilter):?>
					<div class="clb"></div>
					<button class="bx_filter_search_reset btn-link-text font_sxs colored_theme_text_with_hover<?=($bActiveFilter ? '' : ' hidden');?>" type="reset" id="del_filter" name="del_filter" data-href="">
					...
					<div class="bx_filter_button_box active hidden colored_theme_bg">
						<div class="bx_filter_block">
							<div class="bx_filter_parameters_box_container clearfix">
								...
								<button class="bx_filter_search_reset btn btn-default btn-transparent round-ignore<?=($bActiveFilter ? '' : ' hidden');?> pull-left" type="reset" id="del_filter1" name="del_filter1" data-href="" title="<?=Loc::getMessage("CT_BCSF_DEL_FILTER")?>">
								...
								<div class="bx_filter_popup_result right pull-left" id="modef_mobile" <?if(!isset($arResult["ELEMENT_COUNT"])) echo 'style="display:none"';?>>
