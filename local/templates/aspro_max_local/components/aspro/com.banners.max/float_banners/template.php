							...
							<div class="col-md-<?=$col;?> col-sm-6 col-xs-6 col-xxs-12 item-width-261">
								<div id="<?=$this->GetEditAreaId($arItem['ID']);?>" class="item hover_zoom">
									<?if(is_array($arItem['PREVIEW_PICTURE']) ):?>
										<div class="image shine rounded3 <?=($arItem['DETAIL_PICTURE'] ? 'hidden-sm hidden-xs' : '')?>">
											<div class="img_inner">
												<?if($sUrl):?>
									...
									<?if(is_array($arItem['DETAIL_PICTURE']) ):?>
										<div class="image shine rounded3 hidden-lg hidden-md">
											<div class="img_inner">
												<?if($sUrl):?>
													<a href="<?=$sUrl;?>" title="<?=($arItem['DETAIL_PICTURE']['TITLE']?$arItem['DETAIL_PICTURE']['TITLE']:$arItem['NAME']);?>">
												<?endif;?>
												<span class="lazy<?=$position;?><?=($bType2 ? ' bg-fon-img darken-bg-animate' : '');?>" data-src="<?=$arItem['DETAIL_PICTURE']['SRC']?>" style="background-image:url(<?=\Aspro\Functions\CAsproMax::showBlankImg($arItem['DETAIL_PICTURE']['SRC']);?>)"></span>
												<?if($sUrl):?>
													</a>
												<?endif;?>
											</div>
