...
<div class="<? if ($_POST['ajax'] == 'Y') { ?>d-flex wide mx-auto px-4 py-5<? } else { ?>row<? } ?>" id="<?= $itemIds['ID'] ?>" itemscope itemtype="http://schema.org/Product">
	<div class="w-100 d-flex flex-wrap">
		...
		?>
		<div class="item element w-100 d-flex flex-wrap" data-id="<?= $arResult["ID"] ?>" data-price="<?= $price['PRICE'] ?>" data-name="<?= $name ?>" data-name-code="<?= $namecode ?>" data-color="<?= $arResult["PROPERTIES"]['TSVET']['VALUE'] ?>" data-preview="<?= $arResult['PREVIEW_PICTURE']['SRC'] ?>" data-size="<?= $RAZMER ?>" data-size-code="<?= $RAZMERCODE ?>">
			...
			<div class="col-12 <? if ($_POST['ajax'] == 'Y') { ?>col-md-9<? } else { ?>col-md-6 mt-md-4<? } ?> order-1 order-md-1">
				<div class="row sliders">
						...
						<div class="slider-controls">
							...
							?>
										<div class="product-item-detail-slider-controls-block" id="<?= $itemIds['SLIDER_CONT_OF_ID'] . $offer['ID'] ?>" style="display: <?= $strVisible ?>;">
											...
											?>
												<div class="product-item-detail-slider-controls-image<?= ($key == 0 ? ' active current' : '') ?> mb-1" data-entity="slider-control" data-value="<?= $offer['ID'] . '_' . $photo['ID'] ?>" data-slide="<?= $key ?>" data-slide-current="<? if ($key == '0') { ?>first<? } ?><? if ($key == '2') { ?>third<? } ?>">
													<img src="<?= $photo['SRC'] ?>">
												</div>
											<?
												if ($key > 2) {
									...
									<div class="product-item-detail-slider-controls-block position-relative" id="<?= $itemIds['SLIDER_CONT_ID'] ?>">
										...
										if (!empty($actualItem['MORE_PHOTO'])) {
											...
											foreach ($actualItem['MORE_PHOTO'] as $key => $photo) {
												$count++;
										?>
												<div class="product-item-detail-slider-controls-image<?= ($key == 0 ? ' active current' : '') ?> mb-1" onmouseover="slidercontrolsimg()" data-entity="slider-control" data-value="<?= $photo['ID'] ?>" data-slide="<?= $key ?>">
													<img src="<?= $photo['SRC'] ?>">
												</div>
											<?
											}
					...
					<div class="tmb-wrap slider__col not-mobil">
						<div class="slider-control slider__prev <? if (count($actualItem['MORE_PHOTO']) < 4) : ?>hide<? endif; ?> "><svg fill="#262626" width="17" height="10" viewBox="0 0 17 10" xmlns="http://www.w3.org/2000/svg">
								<path d="M8.50003 -7.98981e-05C8.8047 -7.98714e-05 9.10934 0.116252 9.34163 0.348426L16.6513 7.65814C17.1162 8.12313 17.1162 8.87703 16.6513 9.34183C16.1865 9.80663 15.4327 9.80663 14.9677 9.34183L8.50003 2.87379L2.03233 9.3416C1.56735 9.8064 0.813674 9.8064 0.348911 9.3416C-0.116302 8.8768 -0.116302 8.1229 0.348911 7.65791L7.65843 0.3482C7.89083 0.115988 8.19547 -7.99247e-05 8.50003 -7.98981e-05Z"></path>
							</svg></div>
						...
						<div class="slider-control slider__next <? if (count($actualItem['MORE_PHOTO']) < 4) : ?>hide<? endif; ?>"><svg fill="#262626" width="17" height="10" viewBox="0 0 17 10" xmlns="http://www.w3.org/2000/svg">
								<path d="M8.49997 9.69051C8.1953 9.69051 7.89066 9.57418 7.65837 9.342L0.348741 2.03229C-0.116247 1.5673 -0.116247 0.813401 0.348741 0.3486C0.813541 -0.1162 1.56729 -0.1162 2.03231 0.3486L8.49997 6.81664L14.9677 0.348826C15.4327 -0.115974 16.1863 -0.115974 16.6511 0.348826C17.1163 0.813627 17.1163 1.56753 16.6511 2.03251L9.34157 9.34223C9.10917 9.57444 8.80453 9.69051 8.49997 9.69051Z"></path>
							</svg></div>
					</div>
					<div class="product-image d-md-block d-none position-relative col">
						...
						<div style="cursor: zoom-in;" class="col2 not-mobil ">
							<span class="product-item-detail-slider-left  slider__prev" data-entity="slider-control-left"></span>
							<div class="slider__images ">
								<div class="wrap swiper-container">
									<div class="slider swiper-wrapper slider-new  ">
										<?
										if (!empty($actualItem['MORE_PHOTO'])) {
											foreach ($actualItem['MORE_PHOTO'] as $key => $photo) {
										?>
												<div class="swiper-slide">
													<div class="zoom lider__image " id=" slider-img" data-entity="image" data-id="<?= $photo['ID'] ?>">
														<a class="zoom-fancy" itemprop="contentUrl" data-fancybox="images" href="<?= $photo['SRC'] ?>">
															<img src="<?= $photo['SRC'] ?>" alt="<?= $alt ?>" title="<?= $title ?>" <?= ($key == 0 ? ' itemprop="image"' : '') ?>>
														</a>
													</div>
												</div>
							...
							<span class="product-item-detail-slider-right slider__next " data-entity="slider-control-right" style=""></span>
						</div>
					</div>
				</div>
			</div>
							...
							?>
									...
									case 'quantity':
										if ($arParams['USE_PRODUCT_QUANTITY']) {
											...
											<?
											$quantity = "";
											if((float)$actualItem['PRODUCT']['QUANTITY']){
												$quantity = "data-quantity='".(float)$actualItem['PRODUCT']['QUANTITY']."'";
											}
											?>
											...
											<div class="quantity w-100 <?= ($actualItem['CAN_BUY'] ? 'd-md-block d-flex' : 'd-none') ?> align-items-center" <?= (!$actualItem['CAN_BUY'] ? ' style="display: none;"' : '') ?> data-entity="quantity-block">
												...
												<div class="value d-flex w-100 mt-md-2 mb-md-3">
													...
													<span class="product-item-amount-field-btn-plus no-select btn border-gold round d-flex justify-content-center align-items-center" id="<?= $itemIds['QUANTITY_UP_ID'] ?>" <?=$quantity?>>
...
<script type="text/javascript">
	$(document).ready(function(){
		$('.product-item-amount-field-btn-plus').on('click',function(){
			...
			if(Number(count) < $('.product-item-amount-field-btn-plus').data('quantity')){
				count++;
			}else{
				count = $('.product-item-amount-field-btn-plus').data('quantity');
			}
		...
		$('.product-item-amount-field').on('input',function(){
			if($(this).val() > $('.product-item-amount-field-btn-plus').data('quantity')){
				$(this).val($('.product-item-amount-field-btn-plus').data('quantity'));
			}
		})
	})
</script>