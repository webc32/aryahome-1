var	deliveryTypeOne = ["8","12","15"],
	deliveryTypeTwo = ["9","11","14"],
	deliveryTypeOneProps = ["30"],
	deliveryTypeTwoProps = ["22","23","24","25","26","46"];

BX.ready(function(){
    BX.bindDelegate(BX('bx-soa-delivery'),'click',{className: 'type'},function(e){
    	$(this).parents('.section').first().find('.type').removeClass('active');
        $(this).addClass('active');

        if ($(this).attr('data-tab') == 1) {
        	$('#bx-soa-delivery .courier h2').css('display','none');
        	$('#bx-soa-delivery .pvz h2').css('display','block');
        	deliveryTypeOne.forEach((element) => {
	            $('.deliveryId_'+element).css('display','flex');
	        });
    		deliveryTypeTwo.forEach((element) => {
	            $('.deliveryId_'+element).css('display','none');
	        });
	        deliveryTypeOneProps.forEach((element) => {
	            $('input[name="ORDER_PROP_'+element+'"]').parents('.form-group').first().css('display','block');
	            $('input[name="ORDER_PROP_'+element+'"]').val('');
	        });
    		deliveryTypeTwoProps.forEach((element) => {
	            $('input[name="ORDER_PROP_'+element+'"]').parents('.form-group').first().css('display','none');
	            if (element != '22') {$('input[name="ORDER_PROP_'+element+'"]').val('');}
	        });
	        $( '.deliveryId_8' ).click();
        }
        else if($(this).attr('data-tab') == 2){
        	$('#bx-soa-delivery .courier h2').css('display','block');
        	$('#bx-soa-delivery .pvz h2').css('display','none');
        	deliveryTypeOne.forEach((element) => {
	            $('.deliveryId_'+element).css('display','none');
	        });
	        deliveryTypeTwo.forEach((element) => {
    ...
    var clickHandler = BX.bindDelegate(document, 'bxchange', {id: 'address'}, function()
    {
        var obj = $('input.bx-ui-sls-fake[type=text]');
        obj.each(function(){
            ...
            if ($this !== undefined) {
                var town = '',
                	...
                	apart = '',
                	block = '';
                if ($('[name=ORDER_PROP_22]').val() != '') {
                	town = $('[name=ORDER_PROP_22]').val();}
                if ($('[name=ORDER_PROP_23]').val() != '') {
                	street = 'ул. '+$('[name=ORDER_PROP_23]').val();}
                if ($('[name=ORDER_PROP_24]').val() != '') {
                	house = 'д. '+$('[name=ORDER_PROP_24]').val();}
                if ($('[name=ORDER_PROP_26]').val() != '') {
                	apart = 'кв. '+$('[name=ORDER_PROP_26]').val();}
                if ($('[name=ORDER_PROP_25]').val() != '') {
                	block = 'Корпус/строение '+$('[name=ORDER_PROP_25]').val();}
                $("[name=ORDER_PROP_7]").val(town+', '+street+', '+house+', '+apart+', '+block);
            }       
        });
    });
    //Автозаполнение фио
    var clickHandler = BX.bindDelegate(document.body, 'bxchange', {id: 'soa-property-1'}, function()
    {
        var name = '',
        	surname = '';
        if ($('[name=ORDER_PROP_47]').val() != '') {
        	name = $('[name=ORDER_PROP_47]').val();}
        if ($('[name=ORDER_PROP_48]').val() != '') {
        	surname = $('[name=ORDER_PROP_48]').val();}
        $("[name=ORDER_PROP_1]").val(name+' '+surname);
    });
    //Комментарии
    var clickHandler = BX.bindDelegate(document.body, 'bxchange', {id: 'orderDescription'}, function()
    {
        var comment = '';
        if ($('[name=DESCRIPTION]').val() != '') {
        	comment = $('[name=DESCRIPTION]').val();}
        $("[name=ORDER_DESCRIPTION]").val(comment);
    });
    //Автозаполнения выбора ПВЗ
    var clickHandler = BX.bindDelegate(document.body, 'bxchange', {id: 'soa-property-1'}, function()
    {
    	if ($('[name=ORDER_PROP_30]').val() != '') {
        	$('.PVZinput').html($('[name=ORDER_PROP_30]').val());
        }
    });
})
...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		{
			var name = $('#bx-soa-properties').find('input[name=ORDER_PROP_1]');
			if (name.val() == ''){name.parent().addClass('error');}else{name.parent().removeClass('error');}
			
			if ($('.catalog .order #bx-soa-delivery .type[data-tab=2]').hasClass('active')) {
				var delivery = $('#bx-soa-delivery');
				var town = delivery.find('input[name=ORDER_PROP_22]');
			    var street = delivery.find('input[name=ORDER_PROP_23]');
			    var house = delivery.find('input[name=ORDER_PROP_24]');

			    if (town.val() == ''){town.parent().addClass('error');}else{town.parent().removeClass('error');}
			    if (street.val() == ''){street.parent().addClass('error');}else{street.parent().removeClass('error');}
			    if (house.val() == ''){house.parent().addClass('error');}else{house.parent().removeClass('error');}

			    if (delivery.find('.error').length == 0){
			        if (this.isValidForm()){
						this.allowOrderSave();
						...
						}else{this.doSaveAction();}}
			    }
			}else if($('.catalog .order #bx-soa-delivery .type[data-tab=1]').hasClass('active')) {
				var delivery = $('#bx-soa-delivery');
			    var pvz = delivery.find('input[name=ORDER_PROP_30]');

			    if (pvz.val() == ''){pvz.parent().addClass('error');}else{pvz.parent().removeClass('error');}
		...
		getDeliveryLocationInput: function(node)
		{
			...
			if (altProperty)
			{
				...
				labelTextHtml = altProperty.REQUIRED == 'Y' ? '<span class="bx-authform-starrequired text-red">*</span> ' : '';
				labelTextHtml += BX.util.htmlspecialchars(altProperty.NAME);

				label = BX.create('LABEL', {
					attrs: {for: 'altProperty'},
					props: {className: 'bx-soa-custom-label w-100 mt-2'},
					html: labelTextHtml
				});
				...
				input = BX.create('INPUT', {
					props: {
						...
						autocomplete: 'city',
						className: 'form-control bx-soa-customer-input bx-ios-fix border-gray px-4',
		...
		getZipLocationInput: function(node)
		{
			...
			if (zipProperty)
			{
				...
				labelTextHtml = zipProperty.REQUIRED == 'Y' ? '<span class="bx-authform-starrequired text-red">*</span> ' : '';
				...
				label = BX.create('LABEL', {
					...
					style: {display: 'none'},
					props: {className: 'bx-soa-custom-label w-100'},
					html: labelTextHtml
				});
				input = BX.create('INPUT', {
					props: {
		...
		editActivePaySystemBlock: function(activeNodeMode)
		{
			...
			else
			{
				...
				paySystemNode = BX.create('DIV', {props: {className: 'bx-soa-pp container-fluid'}});
		...
		editPaySystemItems: function(paySystemNode)
		{
            ...
            var paySystemItemsContainer = BX.create('DIV', {props: {className: 'd-flex flex-wrap w-100 mb-4'}});
		...
		createPaySystemItem: function(item)
		{
                ...
                children: [
                    ...
                    BX.create('INPUT', {
                        props: {
                            ...
                            name: 'PAY_SYSTEM_ID',
                            type: 'checkbox',
                            className: 'bx-soa-pp-company-checkbox d-none',
                            value: paySystemId,
                            checked: checked
                        }
                    })
            ...
            {
                title = BX.create('DIV', {props: {className: 'bx-soa-pp-company-smalltitle title-5 font-weight-500 col pl-4 pt-1'}, text: item.NAME});
            }

            itemNode = BX.create('DIV', {
                props: {className: 'bx-soa-pp-company d-flex align-items-center mx-2 ml-md-0'},
		...
		editPaySystemInfo: function(paySystemNode)
		{
			...
			var	innerPs, extPs, delimiter, currentPaySystem,
			...
			if (currentPaySystem)
			{
				...
				if (currentPaySystem.PRICE && parseFloat(currentPaySystem.PRICE) > 0)
				{
					price = BX.create('DIV', {
						...
						children: [
							BX.create('SPAN', {props: {className: 'bx-soa-pp-list-termin'}, html: this.params.MESS_PRICE + ':'}),
							BX.create('SPAN', {props: {className: 'bx-soa-pp-list-description'}, text: '~' + currentPaySystem.PRICE_FORMATTED})
						]
					});
				}

                extPs = BX.create('DIV', {children: [price, title]});
			}
		...
		editActiveDeliveryBlock: function(activeNodeMode)
		{
			...
			else
			{
                ...
                deliveryNode = BX.create('DIV', {props: {className: 'w-100'}});
		...
		editDeliveryItems: function(deliveryNode)
		{
			...
			var deliveryTypeContainer = BX.create('DIV', {
				props: {className: 'section w-100 d-flex flex-wrap mt-md-2 mb-4'},
				children: [
					BX.create('DIV', {
						props: {className: 'col-md-6 col-12 mb-4'}, 
						children: [
							BX.create('DIV', {
								props: {className: 'row d-block mx-md-2'},
								children: [
									BX.create('DIV', {
										props: {className: 'type w-100 d-flex flex-wrap align-items-center px-4 py-2',id: 'deliverytype'},
										attrs: {'data-tab': '1'},
										children: [
											BX.create('SPAN', {
												props: {className: 'ellipse'},
												html: '<svg fill="#E1E1E1" width="24" height="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><circle cx="12" cy="12" r="11.5" fill="white" stroke="#E1E1E1"></circle><circle cx="12" cy="12" r="5"></circle></svg>'
											}),
											BX.create('SPAN', {
												props: {className: 'icon mx-3'},
												html: '<svg fill="#D0A550" width="36" height="36" viewBox="0 0 36 36" xmlns="http://www.w3.org/2000/svg"><g clip-path="url(#clip3)"><path d="M31.0455 9.35773C29.7773 5.25936 26.4433 1.92437 22.3449 0.655999C18.0072 -0.687673 13.392 0.0591162 9.8693 2.65477C6.37252 5.2325 4.28516 9.36181 4.28516 13.7004C4.28516 16.6934 5.23198 19.5369 7.02263 21.923L18.0002 36L28.9777 21.9219C31.6575 18.3489 32.4117 13.7694 31.0455 9.35773Z"></path><g clip-path="url(#clip4)"><path d="M10.5801 7.77828L17.9988 10.4834L25.4175 7.77828L17.9988 5.07324L10.5801 7.77828Z" fill="white"></path><path d="M27.2306 8.64648L19.0059 11.6454L21.0203 15.1653L29.2451 12.1663L27.2306 8.64648Z" fill="white"></path><path d="M17.3417 13.7598L15.5457 16.8979L9.14648 14.5646V19.3972L17.3417 22.3854V13.7598Z" fill="white"></path><path d="M20.4542 16.8979L18.6582 13.7598V22.3854L26.8535 19.3972V14.5646L20.4542 16.8979Z" fill="white"></path><path d="M14.9787 15.1653L16.9931 11.6454L8.7684 8.64648L6.75391 12.1663L14.9787 15.1653Z" fill="white"></path></g></g><defs><clipPath id="clip3"><rect width="36" height="36" fill="white"></rect></clipPath><clipPath id="clip4"><rect width="22.5" height="24.75" fill="white" transform="translate(6.75 -1.125)"></rect></clipPath></defs></svg>'
											}),
											BX.create('SPAN', {
												props: {className: 'title-5 font-weight-500'},
												html: 'Пункты выдачи'
											})
										]
									})
								...
								children: [
									BX.create('DIV', {
										...
										children: [
											...
											BX.create('SPAN', {
												props: {className: 'icon mx-3'},
												html: '<svg fill="#D0A550" width="39" height="36" viewBox="0 0 39 23" xmlns="http://www.w3.org/2000/svg"><path d="M37.4675 11.9919L33.7067 11.4212L32.1864 7.16238C31.7863 6.02084 30.7861 5.31836 29.5458 5.31836H25.1849C24.8248 5.31836 24.5048 5.58179 24.4648 5.97694L23.0245 14.7141C22.9444 15.1092 22.6644 15.3726 22.3043 15.3726H3.18026C2.82019 15.3726 2.50012 15.6361 2.46011 16.0312L2.06003 18.5338C1.98001 19.0168 2.34009 19.4997 2.78018 19.4997H4.34051C4.18047 21.3877 5.38073 22.8804 7.10109 22.8804C8.82146 22.8804 10.5418 21.3877 10.9819 19.4997H28.3856C28.2256 21.3877 29.4258 22.8804 31.1462 22.8804C32.9065 22.8804 34.5869 21.3877 35.027 19.4997H36.9074C37.2675 19.4997 37.5875 19.2363 37.6275 18.8412L38.4677 13.6164C38.6278 12.7822 38.1877 12.0798 37.4675 11.9919ZM9.46159 19.1924C9.30156 20.2461 8.38136 21.1242 7.38115 21.1242C6.42095 21.1242 5.7408 20.2461 5.94085 19.1924C6.10088 18.1387 7.02107 17.2606 8.02129 17.2606C8.98149 17.2606 9.66163 18.1387 9.46159 19.1924ZM25.9851 11.2895L26.5852 7.60143H29.1457C29.5858 7.60143 29.9859 7.86486 30.146 8.30391L31.2262 11.2895H25.9851ZM33.5067 19.1924C33.3466 20.2461 32.4264 21.1242 31.4262 21.1242C30.426 21.1242 29.7859 20.2461 29.9859 19.1924C30.146 18.1387 31.0662 17.2606 32.0664 17.2606C33.0666 17.2606 33.6667 18.1387 33.5067 19.1924Z"></path><path d="M22.1446 13.528L24.105 1.3663C24.185 0.883348 23.8249 0.400391 23.3848 0.400391H4.3008C3.62066 0.400391 3.06054 1.01506 3.06054 1.76145C3.06054 2.50784 3.62066 3.12251 4.3008 3.12251H4.50084H6.38124C7.06139 3.12251 7.6215 3.73718 7.6215 4.48357C7.6215 5.22996 7.06139 5.84463 6.38124 5.84463H3.02053C2.34039 5.84463 1.78027 6.45931 1.78027 7.20569C1.78027 7.95208 2.34039 8.56675 3.02053 8.56675H5.10097C5.78112 8.56675 6.34123 9.18143 6.34123 9.92781C6.34123 10.6742 5.78112 11.2889 5.10097 11.3328H1.74026C1.06012 11.3328 0.5 11.9475 0.5 12.6938C0.5 13.4402 1.06012 14.0549 1.74026 14.0549H21.3844C21.7845 14.2305 22.1046 13.9232 22.1446 13.528Z"></path></svg>'
											}),
											BX.create('SPAN', {
												props: {className: 'title-5 font-weight-500'},
												html: 'Доставка курьером'
											})
										]
									})
								]
			...
			deliveryNode.appendChild(deliveryTypeContainer);

			var deliveryPropsContainer = BX.create('DIV', {props: {className: 'bx-soa-delivery courier d-flex flex-wrap w-100'}}),
				group, property, groupIterator = this.propertyCollection.getGroupIterator(), propsIterator;

			deliveryPropsContainer.appendChild(
				BX.create('h2', {
					props: {className: 'title-5 low font-weight-bold col-12'},
					html: 'Адрес доставки'
				})
			);

			if (!deliveryPropsContainer)
				deliveryPropsContainer = this.propsBlockNode.querySelector('.w-100.bx-soa-delivery');
			...
			while (group = groupIterator())
			{
				...
				{
					//Вывести свойства в доставку
					if (
						   (property.getName()=='Адрес доставки')
						|| (property.getName()=='Город')
						|| (property.getName()=='Корпус')
						|| (property.getName()=='Улица')
						|| (property.getName()=='Офис/Квартира')
						|| (property.getName()=='Дом')
						|| (property.getName()=='Комментарий к доставке')
						|| (property.getName()=='Дата доставки')
					) {

						this.getPropertyRowNode(property, deliveryPropsContainer, false);
						deliveryNode.appendChild(deliveryPropsContainer);

					}
				}
			}

			var deliveryItemsContainer = BX.create('DIV', {props: {className: 'order-md-1 order-2 col-12 bx-soa-pp-item-container'}}),
				deliveryItemsContainerRow = BX.create('DIV', {props: {className: 'position-relative'}}),
				deliveryItemNode, k;
			...
			deliveryItemsContainer.appendChild(
				BX.create('h2', {
					props: {className: 'title-5 low font-weight-bold'},
					html: 'Выберите службу доставки'
				})
			);
			...
			var deliveryPropsPVZContainer = BX.create('DIV', {props: {className: 'bx-soa-delivery pvz d-flex flex-wrap w-100'}}),
				group, property, groupIterator = this.propertyCollection.getGroupIterator(), propsIterator;

			deliveryPropsPVZContainer.appendChild(
				BX.create('h2', {
					props: {className: 'title-5 low font-weight-bold col-12'},
					html: 'Пункт самовывоза',
				})
			);

			if (!deliveryPropsPVZContainer)
				deliveryPropsPVZContainer = this.propsBlockNode.querySelector('.w-100.bx-soa-delivery');
			...
			while (group = groupIterator())
			{
				...
				while (property = propsIterator())
				{
					//Вывести свойства в доставку
					if (
						   (property.getName()=='Пункт самовывоза')
						|| (property.getName()=='Информация о ПВЗ')
					) {

						this.getPropertyRowNode(property, deliveryPropsPVZContainer, false);
						deliveryNode.appendChild(deliveryPropsPVZContainer);
		...
		editDeliveryInfo: function(deliveryNode)
		{
	        ...
	        $(document).ready(function() {
    			...
    			$('[name=ORDER_PROP_40]').val(arDateDeliveryText);
    			$('[name=ORDER_PROP_41]').val(arDateDeliveryText);
    		});
			...
			if (currentDelivery.PERIOD_TEXT && currentDelivery.PERIOD_TEXT.length)
			{
				period = BX.create('DIV', {
					children: [
						BX.create('SPAN', {props: {className: 'bx-soa-pp-list-termin'}, html: this.params.MESS_PERIOD + ':'}),
						BX.create('SPAN', {props: {className: 'bx-soa-pp-list-description'}, html: currentDelivery.PERIOD_TEXT})
					]
				});
			}
			...
			deliveryInfoContainer.appendChild(
                BX.create('DIV', {
                    props: {className: 'bx-soa-pp-company'},
                    children: [clear, infoList, extraServicesNode]
                })
            );
			
			var currentDeliveryNode = BX.findChild(deliveryNode, {className: 'bx-soa-pp-company bx-selected'}, true);
			var parametersNode;
			var parametersNodes = BX.findChildren(deliveryNode, {className: 'bx-soa-pp-company-parameters'}, true);

			if (parametersNodes && parametersNodes.length)
				for (var i = 0; i < parametersNodes.length; i++)
					BX.cleanNode(parametersNodes[i]);

			if (currentDeliveryNode) {
				parametersNode = BX.findChild(currentDeliveryNode, {className: 'bx-soa-pp-company-parameters'}, true);

				if (parametersNode) {
                    parametersNode.appendChild(deliveryInfoContainer);
                }
			}
		...
		createDeliveryItem: function(item)
		{
            ...
            if (this.params.SHOW_DELIVERY_LIST_NAMES == 'Y') {
                ...
                description = BX.create('DIV', {
                    props: {className: 'bx-soa-pp-desc-container w-100 pl-5'},
            ...
            itemNode = BX.create('DIV', {
                props: {className: 'bx-soa-pp-company deliveryId_' + deliveryId + ' flex-wrap align-items-center w-100 mb-4'},
                style: {display: 'flex'},
                children: [label, title],
            ...
            itemNode.appendChild(parameters);
            itemNode.appendChild(description);

            if (checked) {
                BX.addClass(itemNode, 'bx-selected');
                BX.addClass(description, 'bx-soa-pp-company-desc');
            }
		...
		getSelectedDelivery: function()
		{
			...
			if (deliveryCheckbox)
			{
				...
				if (deliveryTypeOne.indexOf(deliveryId) != -1){
	            	$('#bx-soa-delivery .type[data-tab=1]').addClass('active');
            		$('#bx-soa-delivery .type[data-tab=2]').removeClass('active');
            		$('#bx-soa-delivery .courier h2').css('display','none');
        			$('#bx-soa-delivery .pvz h2').css('display','block');
            		deliveryTypeOne.forEach((element) => {
			            $('.deliveryId_'+element).css('display','flex');
	            ...
	            if (deliveryTypeTwo.indexOf(deliveryId) != -1){
	            	$('#bx-soa-delivery .type[data-tab=1]').removeClass('active');
            		$('#bx-soa-delivery .type[data-tab=2]').addClass('active');
            		$('#bx-soa-delivery .courier h2').css('display','block');
        			$('#bx-soa-delivery .pvz h2').css('display','none');
            		deliveryTypeOne.forEach((element) => {
			            $('.deliveryId_'+element).css('display','none');
		...
		editPropsItems: function(propsNode)
		{
			...
			var propsItemsContainer = BX.create('DIV', {props: {className: 'w-100 bx-soa-customer d-flex flex-wrap'}}),
				group, property, groupIterator = this.propertyCollection.getGroupIterator(), propsIterator;
			...
			while (group = groupIterator())
			{
				...
				while (property = propsIterator())
				{
					if (
						...
						//Скрывать свойства с пользовательских полей
						|| property.getName()=='Адрес доставки'
						|| property.getName()=='Город'
						|| property.getName()=='Улица'
						|| property.getName()=='Домофон'
						|| property.getName()=='Корпус'
						|| property.getName()=='Дом'
						|| property.getName()=='Офис/Квартира'
						|| property.getName()=='Комментарий к доставке'
		...
		getPropertyRowNode: function(property, propsItemsContainer, disabled)
		{
			...
			else
			{
				if ((property.getId() == '7') || 
					...
					BX.addClass(propsItemNode, "form-group col-md-6 col-12 mb-3 d-none");
				}else if(property.getId() == '22'){
					BX.addClass(propsItemNode, "form-group col-md-6 col-12 mb-3 order-1 order-md-1");
				}else if(property.getId() == '23'){
					BX.addClass(propsItemNode, "form-group col-md-6 col-12 mb-3 order-2 order-md-1");
				}else if(property.getId() == '24'){
					BX.addClass(propsItemNode, "form-group col-md-6 col-12 mb-3 order-3 order-md-1");
				}else if(property.getId() == '25'){
					BX.addClass(propsItemNode, "form-group col-md-6 col-12 mb-3 order-4 order-md-1");
				}else if(property.getId() == '26'){
					BX.addClass(propsItemNode, "form-group col-md-6 col-12 mb-3 order-5 order-md-1");
				}else if(property.getId() == '46'){
					BX.addClass(propsItemNode, "form-group col-md-6 col-12 mb-3 order-6 order-md-1");
				}else{
					BX.addClass(propsItemNode, "form-group col-md-6 col-12 mb-3");
				}

				if (property.isRequired())
					textHtml += '<span class="bx-authform-starrequired text-red">*</span> ';
				...
				if (property.getId() == '30') {
					label = BX.create('LABEL', {
						attrs: {'for': 'soa-property-' + property.getId()},
						props: {className: 'bx-soa-custom-label d-none'},
						html: ''
					});
				}
				else {
			...
			if(property.getId()==30){// идентификатор скрываемого свойства
			   	var addressInput=propsItemNode.querySelector('input');
			   	addressInput.placeholder='Это поле фиксирует выбранный вариант ПВЗ';
			   	// addressInput.setAttribute('disabled', 'disabled');
			   	addressInput.style='display:none;';
			   	// addressInput.value='';
			   	propsItemNode.appendChild(
				   	BX.create('div', {
						props: {className: 'PVZinput'},
						html: '<div class="text-red">не выбран</div>'
					})
				);
			}
		...
		alterProperty: function(settings, propContainer)
		{
			...
			if (textNode)
			{
				...
				if (textNode.id == 'soa-property-23') {
					textNode.id =  'street' + ' soa-property-23';}
				if (textNode.id == 'soa-property-24') {
					textNode.id = 'house' + ' soa-property-24';}
				if (textNode.id == 'soa-property-26') {
					textNode.id = 'apartament' + ' soa-property-26';}
			...
			for (i = 0; i < inputs.length; i++)
			{
				...
				BX.addClass(inputs[i], 'form-control bx-soa-customer-input bx-ios-fix border-gray px-4 w-100');
			}
			...
			{
				inputs[i].placeholder = settings.DESCRIPTION;
				BX.addClass(inputs[i], 'form-control bx-ios-fix border-gray px-4 w-100');
			}
		...
		validateString: function(input, arProperty, fieldName)
		{
			...
			if (value.length)
			{
				...
				if (value.length > 0 && arProperty.PATTERN && arProperty.PATTERN.length)
				{
					...
					if (!re.test(value))
						errors.push(BX.message('SOA_INVALID_PATTERN'));
				}
			}
		...
		editPropsComment: function(propsNode)
		{
			...
			propsCommentContainer = BX.create('DIV', {props: {className: 'd-none w-100'}});
			label = BX.create('LABEL', {
				attrs: {for: 'orderDescription'},
				props: {className: 'bx-soa-customer-label w-100'},
			...
			input = BX.create('TEXTAREA', {
				props: {
					id: 'orderDescription',
					cols: '4',
					className: 'form-control bx-soa-customer-textarea bx-ios-fix border-gray px-4 w-100',
		...
		editTotalBlock: function()
		{
			...
			{
				priceHtml += '<br><span class="bx-price-old text-gray">' + total.PRICE_WITHOUT_DISCOUNT + '</span>';
			}
			...
			this.totalInfoBlockNode.appendChild(
				BX.create('DIV', {
					props: {className: 'bx-soa-cart-total-button-container d-block mt-4' + (!showOrderButton ? ' d-block d-sm-none' : '')},
					children: [
						BX.create('A', {
							props: {
								href: 'javascript:void(0)',
								className: 'btn d-block bg-active round text-uppercase text-center text-white w-100 py-3 px-4 px-md-5 btn-order-save'
							},
		...
		{
			var totalValue, className = 'd-flex justify-content-between';
			...
			if (params.total)
			{
				className += ' bx-soa-cart-total-line-total title-3';
			}
			...
			return BX.create('DIV', {
				...
				children: [
					...
					BX.create('SPAN', {
						props: {
							className: 'bx-soa-cart-d text-right' + (!!params.total && this.options.totalPriceChanged ? ' bx-soa-changeCostSign' : '')
						},
						children: totalValue
					})
				]
