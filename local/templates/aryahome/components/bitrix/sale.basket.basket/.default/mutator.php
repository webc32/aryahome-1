...
foreach ($this->basketItems as $row)
{
    ...
    $SUMM_RC = $row['QUANTITY'] * $RC;

    $skidka = (($row['PRICE'] - $RC)/$RC)*100;
    $skidka = round($skidka);
    $skidka = floor($skidka);

    $res = CIBlockElement::GetProperty(3, $row['PRODUCT_ID'], array("sort" => "asc"), Array("CODE"=>"TSVET_TSENNIKA"));
	while ($ob = $res->GetNext()) {
		if($ob['VALUE_ENUM'] == "Красный"){
			$TSVET_TSENNIKA = 'text-red';   
		}
	}
...
if ($this->hideCoupon !== 'Y' && !empty($result['COUPON_LIST']))
{
	...
	foreach ($totalData['COUPON_LIST'] as &$coupon)
	{
		...
		$discount_count = CSaleDiscount::GetByID($coupon["DISCOUNT_ID"]);
		$totalData['DISCOUNT_COUNT'] = $discount_count["DISCOUNT_VALUE"];
