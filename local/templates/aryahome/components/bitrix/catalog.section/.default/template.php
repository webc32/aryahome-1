...
foreach ($arResult['ITEMS'] as $key  =>$item) {
	$value = $value + $item['CATALOG_PRICE_'.$item[MIN_PRICE][PRICE_ID]];
}
?>
...
<script type="text/javascript">
	//fbq('track', 'Search', {
	//	content_id: ['<?=$item['ID']?>'],
	//	content_category: 'Главная > Каталог',
	//	content_type: 'product'
    //});
</script>

<div class="w-100"> <? // wrapper ?>
	<div class="catalog-section row mb-4 justify-content-md-center" data-entity="<?=$containerName?>">
		<!-- items-container -->
		<?
			...
			foreach ($arResult['ITEM_ROWS'] as $rowData)
			{
				...
				switch ($rowData['VARIANT'])
				{
							...
							$APPLICATION->IncludeComponent(
								...
								array(
									...
									'PARAMS' => $generalParams
                                        ...
                                        + array('HIDE_MICRODATA' => !empty($arParams['HIDE_MICRODATA']) ? $arParams['HIDE_MICRODATA'] : 'N')
								),
								$component,
								array('HIDE_ICONS' => 'Y')
							);
					...
					case 2:
						...
						{
							?>
							<div class="col-sm-4 col-6 product-item-small-card mb-4">
								<?
								if($_REQUEST['ajax']=='Y')
					...
					case 3:
						...
						{
							?>
							<!-- Схема для 4 товаров в ряд -->
							<div class="col-6 col-md-3 product-item-small-card mb-4">
								<?
								$APPLICATION->IncludeComponent(
									'bitrix:catalog.item',
									'.default',
					...
					case 9:
						foreach ($rowItems as $item)
						{
							...
							<div class="col product-item-line-card">
								<? $APPLICATION->IncludeComponent('bitrix:catalog.item', '.default', array(
										'RESULT' => array(
											'ITEM' => $item,
											'AREA_ID' => $areaIds[$item['ID']],
											'TYPE' => $rowData['TYPE'],
	...
	if ($showBottomPager)
	{
		?>
		<div class="">
			<div class="w-100 text-center" data-pagination-num="<?=$navParams['NavNum']?>">
				<!-- pagination-container -->
				<?=$arResult['NAV_STRING']?>
				<!-- pagination-container -->
			</div>
	...
	//endregion
	//region Description
	$pagen = stripos($_SERVER['REQUEST_URI'], 'PAGEN');
	...
	{
		?>
		<div class="row mb-4" id="<?=$user_agent?>">
