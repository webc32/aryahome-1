...
if(is_array($arList)){
	if(array_key_exists('Region',$arList)){
	...
	if(array_key_exists('SelfDelivery',$arList)){
		foreach($arList['SelfDelivery'] as $code => $PVZs){
			if(($allCities || in_array($code,$arParams['CITIES'])) && array_key_exists($code,$arList['Region'])){
				if(
					(!$arParams['LOAD_ACTUAL_PVZ'] || $arParams['LOAD_ACTUAL_PVZ']!='Y' || imlHelper::checkAvPVZ($code,$PVZs)) && 
					(!$arParams['NO_POSTOMAT'] || $arParams['NO_POSTOMAT'] != 'Y' || $PVZs['TYPE'] != 'POSTOMAT')
				){
