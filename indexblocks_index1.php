...
<?if($arMainPageOrder && is_array($arMainPageOrder)):?>
	<?foreach($arMainPageOrder as $key => $optionCode):?>
		...
		<?endif;?>
		<?//CUST_BANNER?>
		<?if($optionCode == "CUST_BANNER"):?>
			<?global $bShowCustbanners, $bShowCustbannersIndexClass;?>
			<?if($bShowCustbanners):?>
				<div class="drag-block container <?=$optionCode?> <?=$bShowCustbannersIndexClass;?> js-load-block" data-class="<?=$subtype?>_drag" data-order="<?=++$key;?>">
					<?=CMax::ShowPageType('mainpage', $subtype, 'type_1');?>
				</div>
			<?endif;?>
		<?endif;?>
		...
		<?if($optionCode == "FAVORIT_ITEM"):?>
			...
			<?if($bShowFavoritItem || ($_SERVER["REMOTE_ADDR"] == "37.112.57.231")):?>
				<div class="drag-block container <?=$optionCode?> js-load-block" data-class="<?=$subtype?>_drag" data-order="<?=++$key;?>" data-file="<?=SITE_DIR;?>include/mainpage/components/<?=$subtype;?>/<?=$strTemplateName;?>.php">
					<?=CMax::ShowPageType('mainpage', $subtype, $strTemplateName);?>
				</div>
			<?endif;?>
		<?endif;?>
